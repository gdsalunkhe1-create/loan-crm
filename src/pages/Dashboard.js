    ...
    if (this.state.hasError) {
      return (
        <div style={{
          ...
          <div style={{fontSize: 32, marginBottom: 12}}>ΓÜá∩╕Å</div>
...
// 'YYYY-MM-DD' for today in IST ΓÇö used only to default/min-bound date pickers.
...
const TEMP_COLORS = {
  Hot: {bg:'#FEE2E2', color:'#DC2626', icon:'≡ƒöÑ'},
  Warm:{bg:'#FEF3C7', color:'#D97706', icon:'ΓÿÇ∩╕Å'},
  Cold:{bg:'#DBEAFE', color:'#2563EB', icon:'Γ¥ä∩╕Å'},
}

// ΓöÇΓöÇ Simple clean theme for the agent-dashboard filter bar ΓöÇΓöÇ
...
// Standard action button ΓÇö icon only, no label (desktop table Actions column)
...
// Lead Stage change should move it ΓÇö the disposition auto-sync stays off.
...
const DEFAULT_WA_TEMPLATES = [
  {
    id:'docs', name:'Docs Reminder', icon:'≡ƒôä', tag:'DOCS',
    tagColor:'#22C55E', tagBg:'#F0FFF4',
    message:`Hi {name} ≡ƒÿè, We have received your loan application for Γé╣{amount}.\n\nTo proceed, please share:\n≡ƒôä PAN Card\n≡ƒôä Aadhaar Card\n≡ƒôä Last 3 months bank statement\n≡ƒôä Latest salary slip\n\nFeel free to reply here if you have any questions!`
  },
  {
    id:'emi', name:'EMI Details', icon:'≡ƒÆ░', tag:'EMI',
    tagColor:'#3B82F6', tagBg:'#EFF6FF',
    message:`Hi {name} ≡ƒÿè, Here are your EMI details for the loan of Γé╣{amount}:\n\n≡ƒÆ░ EMI Amount: Γé╣{emi}/month\n≡ƒôà Tenure: 36 months\n≡ƒôè Interest Rate: 12.5% p.a.\n\nPlease review and confirm to proceed. ≡ƒÖÅ`
  },
  {
    id:'disbursement', name:'Disbursement Update', icon:'≡ƒÄë', tag:'DISBURSED',
    tagColor:'#F59E0B', tagBg:'#FFF7ED',
    message:`≡ƒÄë Congratulations {name}!\n\nYour loan of Γé╣{amount} has been *DISBURSED* Γ£à\n\nThe amount has been credited to your bank account.\n\nThank you for choosing us! ≡ƒÖÅ`
  },
]

// ΓöÇΓöÇΓöÇ MOBILE HOOK ΓöÇΓöÇΓöÇ
...
function AgentDashboard({ userId }) {
  ...
  // navigation or a background data refresh ΓÇö instead of silently resetting to
  ...
  // link backgrounds the browser tab for the actual phone call ΓÇö on low-RAM
  ...
  // saved ΓÇö so a killed tab loses at most a few seconds of typing, not the
  // whole call log.
  ...
  // CALLLOG_DRAFT_KEY for why this can happen ΓÇö a backgrounded tab getting killed
  ...
  // ΓöÇΓöÇ Disbursed-amount capture (Part 2) ΓöÇΓöÇ shared by updateLeadStatus and saveCallLog;
  ...
  // ΓöÇΓöÇ Login-amount manual edit ΓÇö kept fully separate from the disbursed-amount state/handlers
  ...
  // ΓöÇΓöÇ Monthly target widget (Parts 3 & 4) ΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ Motivational disbursement popup (Part 5) ΓöÇΓöÇ
  const [disbursementCelebration,setDisbursementCelebration] = useState(null)

  // ΓöÇΓöÇ Leaderboard (Part 6) ΓöÇΓöÇ
  ...
  useEffect(()=>{
    const build=()=>{
      ...
      if(h>=5&&h<12)  return `Good morning, ${name} ≡ƒæï`
      if(h>=12&&h<17) return `Good afternoon, ${name} ≡ƒæï`
      if(h>=17&&h<21) return `Good evening, ${name} ≡ƒæï`
      return `Good night, ${name} ≡ƒæï`
    }
    setGreeting(build())
  ...
  useEffect(()=>{
    ...
    // 30-second poll ΓÇö refs always point to the latest function so closures stay fresh
  ...
  useEffect(()=>{
    ...
    const onVisible=()=>{
      if(document.visibilityState==='visible'){
        console.log('[RT] tab visible ΓÇö re-syncing')
        fetchAllRef.current?.()
        setLastSynced(new Date())
      }
    }
  ...
  const fetchCallbackTasks=async()=>{
    if(!userId) return
    // Fetch ALL non-terminal callbacks ΓÇö no date restriction, so overdue tasks
    // AND tomorrow's/future callbacks all surface (filtering happens client-side
    // below, never as a DB-side comparison against the timestamptz column ΓÇö
    ...
    // (due_date - 5min has arrived) stays in the list continuously ΓÇö this is
    ...
    // first ΓÇö that's what makes the snooze cadence survive a page refresh ΓÇö
    ...
    if(triggeredTasks.length>0){
      ...
      triggeredTasks.forEach(t=>{
        ...
        supabase.from('tasks').update({last_snooze_at:toDbTimestamp(now)}).eq('id',t.id)
          .then(({error})=>{ if(error) console.error('[Reminder] last_snooze_at write failed ΓÇö has the migration been run? See REMINDER_ENGINE_MIGRATION.sql', error) })
      })
  ...
  const markCallbackDone=async(taskId)=>{
    console.log('[Action] markCallbackDone ΓåÆ DB write task:', taskId)
    ...
    if(!data||data.length===0){ console.warn('[Action] markCallbackDone updated 0 rows ΓÇö RLS blocked or task not found'); showToast('Not saved ΓÇö permission denied on this task','error'); fetchCallbackTasks(); return }
  ...
  const markCallbackAttempted=async(taskId)=>{
    console.log('[Action] markCallbackAttempted ΓåÆ DB write task:', taskId)
    ...
    if(!data||data.length===0){ console.warn('[Action] markCallbackAttempted updated 0 rows ΓÇö RLS blocked or task not found'); showToast('Not saved ΓÇö permission denied on this task','error'); fetchCallbackTasks(); return }
    delete snoozedTasksRef.current[taskId]
    setCallbackTasks(prev=>[...prev.filter(t=>t.id!==taskId)])
  ...
  // column (see timeUtils.js ΓÇö this conversion is the actual bug fix).
  ...
  const rescheduleCallback=async(taskId)=>{
    ...
    console.log('[Action] rescheduleCallback ΓåÆ DB write task:', taskId, 'due:', iso)
  ...
  const scheduleCallback=async()=>{
    ...
    console.log('[Action] scheduleCallback ΓåÆ DB write lead:', callbackLead.id, 'task due:', iso)
    ...
    else if(!lData||lData.length===0){ console.warn('[Action] scheduleCallback lead update hit 0 rows ΓÇö RLS blocked or lead not found'); showToast('Stage not saved ΓÇö permission denied on this lead','error'); fetchAllRef.current?.() }
    ...
    if(tErr){ console.error('[Action] scheduleCallback task insert error:', tErr) }
      ...
      notifyAdmins({type:'callback_set',lead_id:callbackLead.id,customer_name:callbackLead.full_name,due_at:iso,
        message:`ΓÅ░ ${profile?.full_name||'Agent'} set a callback for ${callbackLead.full_name} on ${formatIST(iso)}`,
      })
    }
  ...
  const skipCallback=async()=>{
    ...
    else if(!skipData||skipData.length===0){ console.warn('[Action] skipCallback lead update hit 0 rows ΓÇö RLS blocked or lead not found'); showToast('Stage not saved ΓÇö permission denied on this lead','error'); fetchAllRef.current?.() }
  ...
  const checkReminders=async()=>{
    ...
    // handled by the dedicated popup above) ΓÇö no DB-side date filter, so every
  ...
  const fetchAll=async(opts={})=>{
    ...
    if(fetchInFlightRef.current){
      console.log('[fetchAll] skip ΓÇö a fetch is already in flight')
      return
    }
    ...
    try{
      ...
      if(fetchSeqRef.current!==mySeq){
        console.log('[fetchAll] discarding stale result ΓÇö a newer fetchAll has started since')
        return
      }
  ...
  // and the disbursement celebration popup (Part 5) ΓÇö both read from the same source of truth.
  ...
  const updateLeadStatus=async(leadId,newStatus,disbursedAmountValue)=>{
    ...
    console.log('[Action] updateLeadStatus ΓåÆ DB write:', leadId, newStatus)
    ...
    if(!data||data.length===0){ console.warn('[Action] updateLeadStatus hit 0 rows ΓÇö RLS blocked or lead not found'); showToast('Stage not saved ΓÇö permission denied on this lead','error'); fetchAllRef.current?.(); return }
    // Optimistic update ΓÇö RT subscription will also fire on other devices.
    ...
    if(newStatus==='Lead'){
      ...
      //   message:`≡ƒÄ» ${profile?.full_name||'Agent'} booked a LEAD ΓÇö ${lead.full_name}${lead.loan_amount?' ┬╖ Γé╣'+Number(lead.loan_amount).toLocaleString('en-IN'):''}`,
      // })
    }
  ...
  // Manual correction path for leads already in 'Disbursed' status ΓÇö pre-trigger leads with a
  ...
  const saveDisbursedAmountEdit=async(leadId,amt)=>{
    ...
    setMyLeads(updatedLeads) // re-renders targetProgress/loanAmtDisplay off the same computeTargetProgress(myLeads,ΓÇª) call used elsewhere
  ...
  // Manual correction path for login_amount ΓÇö separate column, separate update call from disbursed_amount above.
  ...
  const saveNote=async()=>{
    ...
    console.log('[Action] saveNote ΓåÆ DB write lead:', noteLead.id)
  ...
  const getObligatedReason=(ob)=>{
    ...
    const isBT=ob.balance_transfer===true||ob.balance_transfer==='true'||ob.balance_transfer==='Yes'||ob.balance_transfer==='yes'
    if(isBT&&obligated===0)return 'BT Applied ΓÇö Not Obligated'
    if(ob.obligation_type==='Credit Card'){
      const out=parseFloat(ob.outstanding_amount)||0
      if(out>0)return `5% of Γé╣${out.toLocaleString('en-IN')} outstanding = Γé╣${Math.round(out*0.05).toLocaleString('en-IN')} obligated`
      return null
    }
    if(obligated===0)return '100% ΓÇö Co-applicant'
    if(obligated===emi*0.5)return '50% ΓÇö Joint / Both'
    return null
  }
  ...
  const saveObligationCard=async(obligation)=>{
    ...
    try{
      if(String(obligation.id).startsWith('tmp-')){
        ...
        if(!data) throw new Error('Insert succeeded but no data returned ΓÇö check RLS SELECT policy on loan_obligations')
        ...
        notifyAdmins({type:'obligation_added',lead_id:leadId,customer_name:_cname,
          ...
          message:`≡ƒôï ${profile?.full_name||'Agent'} added a requirement for ${_cname}${payload.required_loan_amount?' ┬╖ Γé╣'+Number(payload.required_loan_amount).toLocaleString('en-IN'):''}`,
        })
      } else {
        const {data,error}=await supabase.from('loan_obligations').update(payload).eq('id',obligation.id).select().single()
        if(error) throw error
        if(!data) throw new Error('Update succeeded but no data returned ΓÇö check RLS SELECT policy on loan_obligations')
  ...
  const enrichDraftTitle=(ob)=>{
    ...
    const bank=ob.bank_name?` ΓÇö ${ob.bank_name}`:''
    return title+bank
  }
  ...
  const buildObligationShareMessage=()=>{
    ...
    lines.push('')
    lines.push(`Salary: ${obModalNetTakeHome?`Γé╣${Number(obModalNetTakeHome).toLocaleString('en-IN')}`:'Not entered'}`)
    lines.push(`Company: ${obModalCompany||'Not entered'}`)
    lines.push(`Required amount: ${obModalRequiredLoanAmount?`Γé╣${Number(obModalRequiredLoanAmount).toLocaleString('en-IN')}`:'Not entered'}`)
  ...
  const sendWATemplate=(tpl)=>{
    ...
    const entry=`${stamp}: WhatsApp sent ΓÇö "${tpl.name}" template`
  ...
  // Quick one-click WA send (no modal) ΓÇö logs note and opens WA directly
  ...
  const handleCall=(lead)=>{
    ...
    // reference got passed in ΓÇö closures from a list render can lag a beat behind
    ...
    // Don't assign window.location.href directly for tel: ΓÇö on a PC with no
  ...
  const saveCallLog=async(disbursedAmountOverride)=>{
    ...
    try{
      ...
      // Also append the call note into the lead's shared Notes field ΓÇö the same field
      ...
      // here too ΓÇö same reasoning as updateLeadStatus: without it, an out-of-order or
  ...
  // Thin display-formatting aliases over the canonical timeUtils.formatIST ΓÇö
  ...
  // Dynamic stage helpers ΓÇö fall back to module-level constants when table not yet loaded
  ...
  // falls back to displaying the FIRST option in the list ΓÇö which happened to
  // be "Callback" here ΓÇö making an unrelated lead look like it's stuck on
  ...
  const fmtAmt  =n=>n?'Γé╣'+Number(n).toLocaleString('en-IN'):'-'
  // Precedence for the single amount shown in the Leads list / detail modal: disbursed > login > raw import.
  // 'raw' means neither a confirmed disbursed_amount nor login_amount exists yet ΓÇö it's unverified sheet data.
  ...
  // Whether the login-amount pencil should be offered at all ΓÇö true for any lead that has ever
  ...
  // fmtCompactCurrency renders 0 as '-', which reads as "no data" here ΓÇö this widget needs a real zero to read as progress.
  const fmtAchieved=v=>v===0?'Γé╣0':fmtCompactCurrency(v)
  ...
  return (
    <div style={{minHeight:'100vh',background:bg0,color:txt1,fontFamily:'system-ui,sans-serif'}}>
      ...
      {/* POWER DIALER ΓÇö QUEUE COMPLETE */}
      {showWorkspace&&queueComplete&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20}}>
          <div style={{background:'white',borderRadius:20,padding:'40px 48px',maxWidth:480,width:'100%',textAlign:'center',boxShadow:'0 24px 80px rgba(0,0,0,0.3)'}}>
            <div style={{fontSize:56,marginBottom:12}}>≡ƒÄë</div>
      ...
      {/* WA QUICK SEND POPUP ΓÇö desktop: fixed near button, mobile: fixed bottom sheet */}
      {showWAQuick&&(
        <>
          {isMobile?(
            <>
          ...
          <div style={{position:'fixed',top:'50%',left:'50%',transform:'translate(-50%,-50%)',background:'white',borderRadius:16,boxShadow:'0 24px 60px rgba(0,0,0,0.2)',zIndex:500,width:'92%',maxWidth:400,overflow:'hidden'}}>
            <div style={{background:'linear-gradient(135deg,#854F0B,#C05621)',padding:'16px 18px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
              <div style={{display:'flex',alignItems:'center',gap:10}}>
                <div style={{width:36,height:36,borderRadius:'50%',background:'rgba(255,255,255,0.2)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:16}}>≡ƒöö</div>
                <div>
            ...
            <div style={{padding:'16px 18px'}}>
              ...
              <div style={{display:'flex',gap:8}}>
                <button onClick={()=>setReminderPopup(null)} style={{flex:1,padding:10,background:'#185FA5',color:'white',border:'none',borderRadius:8,fontSize:13,fontWeight:600,cursor:'pointer'}}>Γ£à Got it!</button>
                ...
                return(
                  <div key={task.id} style={{border:'1px solid '+(taskOverdue?'#FED7D7':'#FDE68A'),borderLeft:'4px solid '+(taskOverdue?'#DC2626':'#F59E0B'),borderRadius:12,padding:14,marginBottom:12,background:taskOverdue?'#FFF5F5':'#FFFBF0'}}>
                    ...
                    <div style={{fontSize:12,color:taskOverdue?'#B91C1C':'#B45309',marginBottom:task.notes?4:8}}>
                      ≡ƒòÉ Scheduled: {isToday?'Today ':''}{fmtISTDate(task.due_date)}{' '}{fmtISTTime(task.due_date)}
                    </div>
                    {task.notes&&(
          ...
          <div style={{position:'fixed',top:0,right:0,width:isMobile?'100%':340,height:'100vh',background:bg1,boxShadow:'-4px 0 24px rgba(0,0,0,0.15)',zIndex:300,display:'flex',flexDirection:'column'}}>
            ...
            <div style={{flex:1,overflowY:'auto',padding:14}}>
              {callbackTasks.length>0&&(
                <>
                  <div style={{fontSize:11,fontWeight:700,color:'#92400E',textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:6}}>≡ƒô₧ Callbacks Due Today</div>
                  {callbackTasks.map(task=>(
                    <div key={'cb-'+task.id} style={{background:'#FFF7ED',border:'1px solid #FDE68A',borderRadius:8,padding:10,marginBottom:8,borderLeft:'3px solid #F59E0B'}}>
                      ...
                      <div style={{fontSize:11,color:'#B45309'}}>≡ƒòÉ {fmtISTDate(task.due_date)} {fmtISTTime(task.due_date)}</div>
              ...
              {(notifications.length===0&&callbackTasks.length===0)
                ?<div style={{textAlign:'center',padding:'40px 20px',color:'#A0AEC0'}}>
                    ...
                    <div style={{fontSize:13}}>No pending reminders ≡ƒÄë</div>
                  </div>
                :notifications.map(task=>{
                    ...
                    return(
                      <div key={task.id} style={{background:od?'#FFF5F5':'#FFFAF0',border:'1px solid '+(od?'#FED7D7':'#F6E05E'),borderRadius:8,padding:12,marginBottom:8,borderLeft:'3px solid '+(od?'#A32D2D':'#854F0B')}}>
                        <div style={{fontWeight:600,fontSize:12,color:od?'#A32D2D':'#744210',marginBottom:3}}>{od&&'ΓÜá∩╕Å '}{task.title}</div>
                        <div style={{fontSize:11,color:od?'#A32D2D':'#975A16'}}>≡ƒòÉ {fmtISTDate(task.due_date)} {fmtISTTime(task.due_date)}</div>
                      </div>
                    )
                  })
              }
          ...
          <div style={{position:'fixed',top:isMobile?0:'50%',left:isMobile?0:'50%',transform:isMobile?'none':'translate(-50%,-50%)',background:'white',borderRadius:isMobile?0:16,boxShadow:'0 24px 60px rgba(0,0,0,0.18)',zIndex:400,width:isMobile?'100%':'90%',maxWidth:460,height:isMobile?'100%':'auto',maxHeight:isMobile?'100vh':'90vh',overflow:'hidden',display:'flex',flexDirection:'column'}}>
            <div style={{background:'#25D366',padding:'14px 18px',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
              <div style={{display:'flex',alignItems:'center',gap:10}}>
                ...
                <div>
                  <div style={{color:'white',fontWeight:700,fontSize:14}}>WhatsApp ΓÇö {waLead.full_name}</div>
            ...
            <div style={{padding:'16px 18px',overflowY:'auto',flex:1}}>
              <div style={{fontSize:12,color:'#6B7280',marginBottom:12}}>Tap a template ΓÇö name and amount auto-fill:</div>
              {waTemplates.map(tpl=>(
                <div key={tpl.id} onClick={()=>sendWATemplate(tpl)}
                  ...
                  <div style={{fontSize:12,color:'#4B5563',lineHeight:1.5}}>
                    {tpl.message.replace(/{name}/g,waLead.full_name?.split(' ')[0]||'Customer').replace(/{amount}/g,Number(waLead.loan_amount||0).toLocaleString('en-IN')).replace(/{emi}/g,Math.round((Number(waLead.loan_amount||0)*0.012*Math.pow(1.012,36))/(Math.pow(1.012,36)-1)).toLocaleString('en-IN')).slice(0,120)+'ΓÇª'}
                  </div>
                  <div style={{marginTop:8,fontSize:11,color:tpl.tagColor,fontWeight:600}}>Γåù Opens WhatsApp</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <>
          ...
          <div style={{position:'fixed',top:isMobile?'auto':'50%',bottom:isMobile?0:'auto',left:isMobile?0:'50%',transform:isMobile?'none':'translate(-50%,-50%)',background:'white',borderRadius:isMobile?'16px 16px 0 0':16,boxShadow:'0 24px 60px rgba(0,0,0,0.18)',zIndex:400,width:isMobile?'100%':'90%',maxWidth:420,overflow:'hidden'}}>
            <div style={{background:'#854F0B',padding:'14px 18px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
              <div style={{display:'flex',alignItems:'center',gap:8,color:'white',fontWeight:700,fontSize:14}}><IconNotes size={16}/>Add Note ΓÇö {noteLead.full_name}</div>
            ...
            <div style={{padding:'16px 18px'}}>
              ...
              <div style={{marginBottom:14}}>
                ...
                <textarea value={callbackNotes} onChange={e=>setCallbackNotes(e.target.value)}
                  placeholder="Reason for callbackΓÇª"
                  rows={3}
      ...
      {showDisbursedAmountModal&&disbursedAmountLead&&(
        <>
          ...
          <div style={{position:'fixed',top:isMobile?'auto':'50%',bottom:isMobile?0:'auto',left:isMobile?0:'50%',transform:isMobile?'none':'translate(-50%,-50%)',background:'white',borderRadius:isMobile?'16px 16px 0 0':16,boxShadow:'0 24px 60px rgba(0,0,0,0.2)',zIndex:400,width:isMobile?'100%':'90%',maxWidth:420,overflow:'hidden'}}>
            ...
            <div style={{padding:'16px 18px'}}>
              <label style={{display:'block',fontSize:11,fontWeight:600,color:'#6B7280',marginBottom:5,textTransform:'uppercase'}}>Disbursed Amount (Γé╣) *</label>
              ...
              <div style={{display:'flex',gap:8}}>
                <button onClick={confirmDisbursedAmount} disabled={savingDisbursedAmount||!disbursedAmountInput}
                  ...
                  {savingDisbursedAmount?'SavingΓÇª':'Confirm Disbursed'}
                </button>
      ...
      {/* LOGIN AMOUNT MODAL ΓÇö separate from the Disbursed Amount modal above; edits login_amount only */}
      {showLoginAmountModal&&loginAmountLead&&(
        <>
          ...
          <div style={{position:'fixed',top:isMobile?'auto':'50%',bottom:isMobile?0:'auto',left:isMobile?0:'50%',transform:isMobile?'none':'translate(-50%,-50%)',background:'white',borderRadius:isMobile?'16px 16px 0 0':16,boxShadow:'0 24px 60px rgba(0,0,0,0.2)',zIndex:400,width:isMobile?'100%':'90%',maxWidth:420,overflow:'hidden'}}>
            ...
            <div style={{padding:'16px 18px'}}>
              <label style={{display:'block',fontSize:11,fontWeight:600,color:'#6B7280',marginBottom:5,textTransform:'uppercase'}}>Login Amount (Γé╣) *</label>
              ...
              <div style={{display:'flex',gap:8}}>
                <button onClick={confirmLoginAmount} disabled={savingLoginAmount||!loginAmountInput}
                  ...
                  {savingLoginAmount?'SavingΓÇª':'Save Login Amount'}
                </button>
        ...
        <>
          ...
          <div style={{position:'fixed',top:'50%',left:'50%',transform:'translate(-50%,-50%)',background:'white',borderRadius:18,boxShadow:'0 24px 60px rgba(0,0,0,0.25)',zIndex:700,width:isMobile?'92%':420,overflow:'hidden',textAlign:'center'}}>
            <div style={{background:'linear-gradient(135deg,#059669,#10B981)',padding:'26px 20px 20px',color:'white'}}>
              <div style={{fontSize:38,marginBottom:6}}>≡ƒÄë</div>
            ...
            <div style={{padding:'18px 20px'}}>
              {disbursementCelebration.disbursementShortfall>0?(
                <div style={{fontSize:14,color:'#374151',lineHeight:1.6}}>
                  <strong style={{color:'#065F46'}}>{fmtCompactCurrency(disbursementCelebration.disbursementShortfall)}</strong> to go ΓÇö that's{' '}
                ...
                <div style={{fontSize:14,color:'#065F46',fontWeight:600}}>Target achieved ΓÇö amazing work! ≡ƒÅå</div>
              )}
          ...
          <div style={{position:'fixed',top:isMobile?'auto':'50%',bottom:isMobile?0:'auto',left:isMobile?0:'50%',transform:isMobile?'none':'translate(-50%,-50%)',background:'white',borderRadius:isMobile?'16px 16px 0 0':16,boxShadow:'0 24px 60px rgba(0,0,0,0.2)',zIndex:501,width:isMobile?'100%':'90%',maxWidth:460,maxHeight:isMobile?'92vh':'90vh',overflow:'hidden',display:'flex',flexDirection:'column'}}>
            ...
            <div style={{background:'linear-gradient(135deg,#166534,#16a34a)',padding:'14px 18px',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
              <div style={{display:'flex',alignItems:'center',gap:10,color:'white'}}>
                ...
                <div>
                  <div style={{fontWeight:700,fontSize:15}}>Log Call ΓÇö {callLogLead.full_name}</div>
            ...
            <div style={{padding:'16px 18px',overflowY:'auto',flex:1}}>
              ...
              <div style={{marginBottom:12}}>
                ...
                <select value={callLogDisposition} onChange={e=>{
                    ...
                    // share an identically-named Lead Stage option ΓÇö auto-sync the stage to
                  ...
                  onFocus={e=>e.target.style.borderColor='#185FA5'} onBlur={e=>e.target.style.borderColor='#E2E8F0'}>
                  <option value="">ΓÇö Select Disposition ΓÇö</option>
                  ...
                  <div style={{maxHeight:100,overflowY:'auto',border:'1px solid #e5e7eb',borderRadius:8,padding:'6px 10px'}}>
                    {previousOutcomes.map((o,i)=>(
                      <div key={i} style={{fontSize:12,color:'#374151',padding:'4px 0',borderBottom:i<previousOutcomes.length-1?'1px solid #f3f4f6':'none',display:'flex',justifyContent:'space-between'}}>
                        <span style={{fontWeight:600}}>{o.call_outcome||'ΓÇö'}</span>
              ...
              {/* Lead Stage ΓÇö separate from disposition */}
              <div style={{marginBottom:12}}>
              ...
              <div style={{marginBottom:16}}>
                ...
                <textarea value={callLogNotes} onChange={e=>setCallLogNotes(e.target.value)}
                  placeholder="Add call notes hereΓÇª"
                  rows={3}
                  style={{width:'100%',padding:'9px 10px',border:'1.5px solid #E2E8F0',borderRadius:8,fontSize:13,outline:'none',resize:'vertical',fontFamily:'inherit',boxSizing:'border-box',color:'#111827'}}
                  onFocus={e=>e.target.style.borderColor='#185FA5'} onBlur={e=>e.target.style.borderColor='#E2E8F0'}/>
              </div>
              ...
              <div style={{display:'flex',gap:8}}>
                <button onClick={saveCallLog} disabled={savingCallLog}
                  ...
                  <IconDeviceFloppy size={15}/>{savingCallLog?'SavingΓÇª':'Save Call Log'}
                </button>
      ...
      {showObligationModal&&selectedLeadForObligations&&(
        ...
        <div style={{position:'fixed',top:0,bottom:0,right:0,left:isMobile?0:'var(--sidebar-width)',background:'#0f172a',zIndex:360,display:'flex',flexDirection:'column',overflow:'hidden'}}>
              ...
              {!obModalReadOnly&&(isObEditing?(
                <>
                  <button onClick={async()=>{
                    ...
                    const netTakeHomeVal=obModalNetTakeHome===''?null:parseFloat(obModalNetTakeHome)||null
                    const requiredLoanAmountVal=obModalRequiredLoanAmount===''?null:parseFloat(obModalRequiredLoanAmount)||null
                    console.log('[Action] obModal save info ΓåÆ DB write lead:', leadId)
          ...
          {/* ΓöÇΓöÇ Reassigned Banner ΓöÇΓöÇ */}
          {leadReassignInfo&&(
            <div style={{background:'#7c2d12',borderBottom:'1px solid #ea580c',padding:'10px 20px',display:'flex',alignItems:'center',gap:10,flexShrink:0}}>
              <span style={{fontSize:18}}>≡ƒöÇ</span>
          ...
          <div style={{flex:1,overflowY:'auto',padding:isMobile?'14px':'20px 24px',background:'#0f172a',color:'#e2e8f0'}}>
            ...
            {/* ΓöÇΓöÇ UPPER ROW: Salary + Company + Notes ΓöÇΓöÇ */}
            <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:16,marginBottom:20}}>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:12}}>
                <div style={{background:'rgba(255,255,255,0.06)',borderRadius:10,padding:'14px 16px',border:'1px solid rgba(255,255,255,0.1)'}}>
                  ...
                  {isObEditing?(
                    ...
                    <div style={{fontSize:15,color:'#ffffff',fontWeight:500}}>{obModalNetTakeHome?`Γé╣${Number(obModalNetTakeHome).toLocaleString('en-IN')}`:'Not entered'}</div>
                  )}
                ...
                <div style={{background:'rgba(255,255,255,0.06)',borderRadius:10,padding:'14px 16px',border:'1px solid rgba(255,255,255,0.1)'}}>
                  ...
                  {isObEditing?(
                    ...
                    <div style={{fontSize:15,color:'#ffffff',fontWeight:500}}>{obModalRequiredLoanAmount?`Γé╣${Number(obModalRequiredLoanAmount).toLocaleString('en-IN')}`:'Not entered'}</div>
                  )}
                </div>
                ...
                {isObEditing&&(
                  <div style={{display:'flex',gap:6}}>
                    <textarea rows={3} value={obModalNoteInput} onChange={e=>setObModalNoteInput(e.target.value)}
                      placeholder="Add a note and press EnterΓÇª"
                      onKeyDown={e=>{
              ...
              <div style={{display:'flex',flexWrap:'wrap',alignItems:'center',justifyContent:'space-between',gap:8,marginBottom:10}}>
                <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr 1fr':'repeat(auto-fit,minmax(110px,1fr))',gap:8,flex:1}}>
                  <div style={{background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:10,padding:'14px 16px',minWidth:110,flex:1}}>
                    ...
                    <div style={{fontSize:20,fontWeight:600,color:'#60A5FA'}}>Γé╣{obligationTotals.totalEMI.toLocaleString('en-IN')}</div>
                  ...
                  <div style={{background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:10,padding:'14px 16px',minWidth:110,flex:1}}>
                    ...
                    <div style={{fontSize:20,fontWeight:600,color:'#F59E0B'}}>Γé╣{obligationTotals.totalObligatedEMI.toLocaleString('en-IN')}</div>
                  ...
                  <div style={{background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:10,padding:'14px 16px',minWidth:110,flex:1}}>
                    ...
                    <div style={{fontSize:20,fontWeight:600,color:'#60A5FA'}}>Γé╣{obligationTotals.totalOutstanding.toLocaleString('en-IN')}</div>
                  </div>
                  <div style={{background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:10,padding:'14px 16px',minWidth:110,flex:1}}>
                  ...
                  <div style={{background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:10,padding:'14px 16px',minWidth:140,flex:1}}>
                    ...
                    {!(obModalNetTakeHome||obModalSalary)
                      ? <div style={{fontSize:20,fontWeight:600,color:'#64748b'}}>ΓÇö</div>
                      : <div style={{fontSize:18,fontWeight:600,color:obligationTotals.eligibleLoan>0?'#34D399':'#ef4444'}}>Γé╣{obligationTotals.eligibleLoan.toLocaleString('en-IN')}</div>
                    }
                    {(obModalNetTakeHome||obModalSalary)&&<div style={{fontSize:11,color:'rgba(255,255,255,0.4)',marginTop:4}}>Avail EMI: Γé╣{Math.round(obligationTotals.availableEMI).toLocaleString('en-IN')}</div>}
                  ...
                  <div style={{background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:10,padding:'14px 16px',minWidth:140,flex:1}}>
                    ...
                    <div style={{fontSize:20,fontWeight:600,color:'#A78BFA'}}>Γé╣{btPosTotal.toLocaleString('en-IN')}</div>
                  ...
                  <div style={{background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:10,padding:'14px 16px',minWidth:140,flex:1}}>
                    ...
                    <div style={{fontSize:20,fontWeight:600,color:'#34D399'}}>Γé╣{amountInHandAfterBT.toLocaleString('en-IN')}</div>
                  </div>
                </div>
                {!obModalReadOnly&&(
                  ...
                  <div key={ob.id} style={{background:'#111827',border:'1px solid #334155',borderRadius:14,padding:12,marginBottom:10}}>
                      ...
                      <div>
                        ...
                        <div style={{fontSize:11,color:'#94a3b8',marginTop:2,display:'flex',flexWrap:'wrap',alignItems:'center',gap:6}}>
                          {ob.obligation_type==='Credit Card'?(()=>{
                            ...
                            const isBT=ob.balance_transfer===true||ob.balance_transfer==='true'||ob.balance_transfer==='Yes'||ob.balance_transfer==='yes'
                            return(<>
                              {cl>0&&<span>Credit Limit Γé╣{cl.toLocaleString('en-IN')}</span>}
                              {cl>0&&out>0&&<span style={{color:'#475569'}}>┬╖</span>}
                              <span>{out>0?`POS Γé╣${out.toLocaleString('en-IN')}`:'POS not set'}</span>
                              {out>0&&(isBT
                                ?<span style={{background:'#14532d',color:'#4ade80',fontSize:10,fontWeight:700,padding:'1px 7px',borderRadius:12}}>BT Applied ΓÇö Not Obligated</span>
                                :<span style={{background:'#431407',color:'#fb923c',fontSize:10,fontWeight:700,padding:'1px 7px',borderRadius:12}}>Obligated Γé╣{Math.round(out*0.05).toLocaleString('en-IN')} (5% of POS)</span>
                              )}
                            </>)
                          })():(()=>{
                            ...
                            return(<>
                              <span>{orig>0?`EMI Γé╣${orig.toLocaleString('en-IN')}`:'EMI not set'}</span>
                              {orig>0&&(obl===0&&reason
                                ...
                                :obl<orig
                                  ?<span style={{background:'#1e3a5f',color:'#93c5fd',fontSize:10,fontWeight:700,padding:'1px 7px',borderRadius:12}}>Obligated Γé╣{obl.toLocaleString('en-IN')} ┬╖ {reason}</span>
                                  :<span style={{background:'#1f2937',color:'#64748b',fontSize:10,fontWeight:600,padding:'1px 7px',borderRadius:12}}>Obligated Γé╣{obl.toLocaleString('en-IN')}</span>
                              )}
                              <span style={{color:'#475569'}}>┬╖</span>
                              <span>{(ob.pos_amount||ob.outstanding_amount)?`Outstanding Γé╣${Number(ob.pos_amount||ob.outstanding_amount).toLocaleString('en-IN')}`:'Outstanding not set'}</span>
                            </>)
                          })()}
                        </div>
                      </div>
                    ...
                    {showExpanded&&obModalReadOnly&&(
                      <div style={{marginTop:10,display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:8,fontSize:12}}>
                        ...
                        <div style={{color:'#64748b'}}>Bank<div style={{color:'#e2e8f0',fontWeight:600,marginTop:2}}>{ob.bank_name||'ΓÇö'}</div></div>
                        <div style={{color:'#64748b'}}>EMI<div style={{color:'#38bdf8',fontWeight:700,marginTop:2}}>{ob.emi_amount?'Γé╣'+Number(ob.emi_amount).toLocaleString('en-IN'):'ΓÇö'}</div></div>
                        <div style={{color:'#64748b'}}>Outstanding<div style={{color:'#a78bfa',fontWeight:700,marginTop:2}}>{(ob.pos_amount||ob.outstanding_amount)?'Γé╣'+Number(ob.pos_amount||ob.outstanding_amount).toLocaleString('en-IN'):'ΓÇö'}</div></div>
                        {ob.obligation_type!=='Personal Loan'&&<div style={{color:'#64748b'}}>Loan Amount<div style={{color:'#e2e8f0',fontWeight:600,marginTop:2}}>{ob.sanctioned_amount?'Γé╣'+Number(ob.sanctioned_amount).toLocaleString('en-IN'):'ΓÇö'}</div></div>}
                        {['Personal Loan','Credit Card'].includes(ob.obligation_type)&&<div style={{color:'#64748b'}}>Balance Transfer<div style={{marginTop:2}}>{ob.balance_transfer?<span style={{color:'#4ade80',fontWeight:700}}>Γ£ô Yes</span>:<span style={{color:'#94a3b8'}}>No</span>}</div></div>}
                    ...
                    {showExpanded&&!obModalReadOnly&&(
                      <div style={{marginTop:12,display:'grid',gridTemplateColumns:isMobile?'1fr':'repeat(2,1fr)',gap:8}}>
                        ...
                        {ob.obligation_type==='Personal Loan'&&(<>
                          ...
                          <div><label style={{display:'block',fontSize:11,fontWeight:700,color:'#94a3b8',marginBottom:4}}>EMI Amount (Γé╣)</label>
                          ...
                          <div><label style={{display:'block',fontSize:11,fontWeight:700,color:'#94a3b8',marginBottom:4}}>EMIs Remaining</label>
                            ...
                            <div style={{fontSize:10,color:'#64748b',marginTop:4}}>Auto-calculated: Tenure ΓêÆ Total EMIs Paid</div></div>
                          ...
                          <div style={{display:'flex',flexDirection:'column',justifyContent:'flex-end'}}>
                            ...
                            <div onClick={()=>!obModalReadOnly&&handleObligationFieldChange(ob.id,'balance_transfer',!ob.balance_transfer)}
                              ...
                              <div style={{width:18,height:18,borderRadius:5,background:ob.balance_transfer?'#16a34a':'#334155',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                                {ob.balance_transfer&&<span style={{color:'white',fontSize:12,fontWeight:700}}>Γ£ô</span>}
                              </div>
                              <span style={{fontSize:12,fontWeight:700,color:ob.balance_transfer?'#4ade80':'#94a3b8'}}>{ob.balance_transfer?'Yes ΓÇö BT':'No'}</span>
                            </div>
                            <div style={{fontSize:10,color:'#64748b',marginTop:4}}>BT loans are excluded at 0% in FOIR</div>
                          </div>
                        </>)}

                        {/* ΓöÇΓöÇ HOUSING LOAN ΓöÇΓöÇ */}
                        {ob.obligation_type==='Housing Loan'&&(<>
                          <div><label style={{display:'block',fontSize:11,fontWeight:700,color:'#94a3b8',marginBottom:4}}>Loan Amount (Γé╣)</label>
                          ...
                          {ob.joint_type==='Joint'&&(<>
                            ...
                            <div><label style={{display:'block',fontSize:11,fontWeight:700,color:'#94a3b8',marginBottom:4}}>Income of Joint Holder (Γé╣)</label>
                        ...
                        {/* ΓöÇΓöÇ CREDIT CARD ΓöÇΓöÇ */}
                        {ob.obligation_type==='Credit Card'&&(<>
                          <div><label style={{display:'block',fontSize:11,fontWeight:700,color:'#94a3b8',marginBottom:4}}>Bank Name</label>
                            <input value={ob.bank_name||''} onChange={e=>handleObligationFieldChange(ob.id,'bank_name',e.target.value)} style={{width:'100%',padding:'7px 10px',borderRadius:8,border:'1px solid #334155',background:'#0f172a',color:'white',outline:'none',fontSize:13}}/></div>
                          <div>
                            <label style={{display:'block',fontSize:11,fontWeight:700,color:'#94a3b8',marginBottom:4}}>Credit Limit (Γé╣)</label>
                          ...
                          <div>
                            ...
                            {(ob.pos_amount||ob.outstanding_amount)&&!ob.balance_transfer&&<div style={{fontSize:10,color:'#f97316',marginTop:3}}>Obligated: Γé╣{Math.round((parseFloat(ob.pos_amount)||parseFloat(ob.outstanding_amount)||0)*0.05).toLocaleString('en-IN')} (5% of POS)</div>}
                        ...
                        {/* ΓöÇΓöÇ CAR LOAN / BIKE LOAN / CONSUMER DURABLE ΓöÇΓöÇ */}
                        ...
                        {ob.obligation_type==='Gold Loan'&&(<>
                          ...
                          <div><label style={{display:'block',fontSize:11,fontWeight:700,color:'#94a3b8',marginBottom:4}}>EMI Amount (Γé╣) <span style={{fontWeight:400,color:'#64748b'}}>= 1% of Loan Amt</span></label>
                            <input type="number" min="0" value={ob.emi_amount||''} onChange={e=>handleObligationFieldChange(ob.id,'emi_amount',e.target.value)} style={{width:'100%',padding:'7px 10px',borderRadius:8,border:'1px solid #334155',background:'#0f172a',color:'white',outline:'none',fontSize:13}}/></div>
                          <div><label style={{display:'block',fontSize:11,fontWeight:700,color:'#94a3b8',marginBottom:4}}>POS (Principal Outstanding)</label>
                            <input type="number" min="0" placeholder="Enter POS amount" value={ob.pos_amount||''} onChange={e=>handleObligationFieldChange(ob.id,'pos_amount',e.target.value)} style={{width:'100%',padding:'7px 10px',borderRadius:8,border:'1px solid #334155',background:'#0f172a',color:'white',outline:'none',fontSize:13}}/>
              ...
              {/* ΓöÇΓöÇ Credit Report (read-only) ΓöÇΓöÇ */}
                ...
                {creditReport?(
                  <div style={{background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:10,padding:'14px 16px'}}>
                    <div style={{display:'flex',flexWrap:'wrap',gap:20,marginBottom:12}}>
                      ...
                      <div><div style={{fontSize:11,color:'rgba(255,255,255,0.5)'}}>Score</div><div style={{fontSize:14,fontWeight:700,color:'#60A5FA',marginTop:2}}>{creditReport.cibil_score??'ΓÇö'}</div></div>
                      <div><div style={{fontSize:11,color:'rgba(255,255,255,0.5)'}}>Report Date</div><div style={{fontSize:14,color:'#e2e8f0',marginTop:2}}>{creditReport.report_date?new Date(creditReport.report_date).toLocaleDateString('en-IN'):'ΓÇö'}</div></div>
                      <div><div style={{fontSize:11,color:'rgba(255,255,255,0.5)'}}>Uploaded</div><div style={{fontSize:14,color:'#e2e8f0',marginTop:2}}>{creditReport.uploaded_at?new Date(creditReport.uploaded_at).toLocaleDateString('en-IN'):'ΓÇö'}</div></div>
                    </div>
                        ...
                        <table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
                          ...
                          <tbody>
                            {creditReport.accounts.map((acc,i)=>(
                              <tr key={i}>
                                <td style={{padding:'6px 8px',color:'#e2e8f0',borderBottom:'1px solid rgba(255,255,255,0.06)'}}>{acc.bankName||'ΓÇö'}</td>
                                <td style={{padding:'6px 8px',color:'#94a3b8',borderBottom:'1px solid rgba(255,255,255,0.06)'}}>{acc.status||'ΓÇö'}</td>
                                <td style={{padding:'6px 8px',color:'#a78bfa',fontWeight:600,borderBottom:'1px solid rgba(255,255,255,0.06)'}}>{acc.outstanding?'Γé╣'+Number(acc.outstanding).toLocaleString('en-IN'):'ΓÇö'}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
          ...
          <div style={{position:'fixed',top:isMobile?0:'50%',left:isMobile?0:'50%',transform:isMobile?'none':'translate(-50%,-50%)',background:'white',borderRadius:isMobile?0:16,zIndex:400,width:isMobile?'100%':'90%',maxWidth:500,height:isMobile?'100%':'auto',maxHeight:isMobile?'100vh':'90vh',overflow:'hidden',display:'flex',flexDirection:'column'}}>
            <div style={{background:'linear-gradient(135deg,#185FA5,#2563EB)',padding:'16px 18px',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
              <div>
                ...
                <div style={{color:'rgba(255,255,255,0.7)',fontSize:12,marginTop:2}}>Filter by stage or date ΓÇö download CSV</div>
              </div>
            ...
            <div style={{padding:16,overflowY:'auto',flex:1}}>
              <div style={{fontSize:13,fontWeight:600,color:'#374151',marginBottom:8}}>Step 1 ΓÇö Select Stage</div>
              ...
              <div style={{fontSize:13,fontWeight:600,color:'#374151',marginBottom:8}}>Step 2 ΓÇö Select Date</div>
              <div style={{display:'flex',gap:7,flexWrap:'wrap',marginBottom:12}}>
                {[{id:'all',label:'All Time'},{id:'today',label:'Today'},{id:'week',label:'This Week'},{id:'month',label:'This Month'},{id:'custom',label:'≡ƒôà Custom'}].map(d=>(
      ...
      {/* ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ TOP BAR ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ */}
      <div style={{background:'linear-gradient(135deg,#0f3460 0%,#185FA5 100%)',padding:isMobile?'12px 14px':'12px 20px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:10,position:'sticky',top:0,zIndex:50}}>
        <div style={{minWidth:0,flex:1}}>
          <div style={{fontSize:isMobile?14:16,fontWeight:700,color:'#ffffff',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{greeting||'Welcome back ≡ƒæï'}</div>
          {!isMobile&&<div style={{fontSize:12,color:'rgba(255,255,255,0.7)',marginTop:2}}>{lastSynced?`Synced ${lastSynced.toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit',timeZone:IST_TZ})} ┬╖ `:''}Ready to close more loans today? ≡ƒÜÇ</div>}
        </div>
        <div style={{display:'flex',alignItems:'center',gap:6,flexShrink:0}}>
          {!isMobile&&(
            <button onClick={()=>setDarkMode(!darkMode)} style={{padding:'7px 10px',background:'rgba(255,255,255,0.12)',border:'1px solid rgba(255,255,255,0.25)',borderRadius:8,cursor:'pointer',fontSize:12,color:'#ffffff'}}>{darkMode?'ΓÿÇ∩╕Å':'≡ƒîÖ'}</button>
          )}
      ...
      {/* ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ CONTENT ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ */}
      <div style={{padding:isMobile?'12px 14px':'16px 20px',maxWidth:1400,margin:'0 auto'}}>
        ...
        {overdueTasks.length>0&&(
          <div style={{background:'#FFF5F5',border:'1px solid #FED7D7',borderRadius:10,padding:'11px 14px',marginBottom:14,display:'flex',alignItems:'center',gap:10}}>
            ...
            <span style={{fontSize:13,color:'#A32D2D',fontWeight:600,flex:1}}>ΓÜá∩╕Å {overdueTasks.length} overdue task{overdueTasks.length>1?'s':''}!</span>
        ...
        {callbackTasks.length>0&&(
          <div style={{background:'#FFF7ED',border:'1px solid #FDE68A',borderRadius:10,padding:'12px 14px',marginBottom:14}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:callbackTasks.length>0?8:0}}>
              <span style={{fontSize:13,fontWeight:700,color:'#92400E'}}>≡ƒô₧ You have {callbackTasks.length} callback{callbackTasks.length>1?'s':''} due today!</span>
        ...
        {/* MONTHLY TARGET WIDGET ΓÇö primary focus of the dashboard, rendered first */}
        {!targetLoading && (
            ...
            {(!monthlyTarget||showEditTarget) ? (
              <>
                {!monthlyTarget && (
                  <div style={{background:'#EEF2FF',border:'1px solid #C7D2FE',borderRadius:8,padding:'8px 12px',fontSize:12.5,fontWeight:600,color:'#3730A3',marginBottom:12}}>
                    ≡ƒÄ» Set your target for {monthNameLabel}
                  </div>
                )}
                <div style={{fontSize:14,fontWeight:700,color:txt1,marginBottom:4}}>{monthlyTarget?'Edit your target':'Monthly Target'} ΓÇö {monthNameLabel}</div>
                ...
                <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:10,marginBottom:12}}>
                  <div>
                    <label style={{display:'block',fontSize:11,fontWeight:600,color:txt2,marginBottom:5,textTransform:'uppercase'}}>Monthly Disbursement Target (Γé╣)</label>
                ...
                <div style={{display:'flex',gap:8}}>
                  <button onClick={saveMonthlyTarget} disabled={savingTarget} className="mobile-button"
                    ...
                    {savingTarget?'SavingΓÇª':'Save Target'}
                  </button>
                  {monthlyTarget&&(
              ...
              <>
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
                  <div style={{fontSize:16,fontWeight:700,color:txt1}}>≡ƒÄ» {monthNameLabel} Target Progress</div>
                ...
                <div style={{background:bg2,borderRadius:8,padding:'12px 14px',fontSize:17,color:txt1,fontWeight:700,lineHeight:1.5}}>
                  {(targetProgress.disbursementShortfall>0||targetProgress.loginShortfall>0)?(
                    ...
                    <>≡ƒÅå Both targets achieved this month!</>
                  )}
                </div>
              </>
            )}
        ...
        {!leaderboardLoading && leaderboard.length>0 && (
          <div style={{background:bg1,border:'1px solid '+bdr,borderRadius:12,padding:16,marginBottom:14,boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
            <div style={{fontSize:14,fontWeight:700,color:txt1,marginBottom:12}}>≡ƒÅå Top Performers ΓÇö {monthNameLabel}</div>
            <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'repeat(3,1fr)',gap:10,marginBottom:leaderboard.length>3?12:0}}>
              {leaderboard.slice(0,3).map((row,i)=>(
                <div key={row.agentId} style={{background:i===0?'#FEF3C7':i===1?'#F1F5F9':'#FBEAD9',border:'1px solid '+(i===0?'#FDE68A':i===1?'#E2E8F0':'#F3D5B5'),borderRadius:10,padding:'12px',textAlign:'center'}}>
                  <div style={{fontSize:22,marginBottom:4}}>{i===0?'≡ƒÑç':i===1?'≡ƒÑê':'≡ƒÑë'}</div>
        ...
        {/* PIPELINE CARDS ΓÇö 2x3 grid (6 cards) */}
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8}}>
          <div style={{fontSize:11,fontWeight:700,color:txt2,textTransform:'uppercase',letterSpacing:'0.6px'}}>≡ƒôè Pipeline Overview</div>
        ...
        {/* STAT CARDS ΓÇö 3 across */}
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8}}>
          <div style={{fontSize:11,fontWeight:700,color:txt2,textTransform:'uppercase',letterSpacing:'0.6px'}}>≡ƒôê Today's Stats</div>
        ...
        <div style={{background:bg1,borderRadius:10,border:'1px solid '+bdr,marginBottom:10,overflow:'hidden'}}>
          <div style={{padding:'10px 14px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:10}}>
            <div style={{display:'flex',alignItems:'center',gap:8}}>
              <span style={{fontSize:13,fontWeight:600,color:txt1}}>≡ƒôÑ Export Leads Report</span>
            </div>
            <button
        ...
        {/* ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ LEADS TAB ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ */}
        {activeTab==='leads'&&(
          <div>
            {/* POWER DIALER BUTTON ΓÇö heavy physical master switch */}
            <div style={{borderRadius:14,padding:'0 0 4px',marginBottom:10,background:'rgba(10,40,40,0.25)',boxShadow:'inset 0 -3px 6px rgba(0,0,0,0.15)'}}>
              <button onClick={startPowerDialer}
                ...
                <IconBolt size={17}/> ΓÜí Start Power Dialer ΓÇö {filteredLeads.length||myLeads.length} Leads
              </button>
            </div>

            {/* FILTER BAR ΓÇö simple clean style */}
            <div style={filterBarWrapStyle}>
              <div style={{flex:'1 1 280px',minWidth:0,position:'relative'}}>
                ...
                <input
                  ...
                  placeholder="Search name, mobileΓÇª"
              ...
              <div style={{position:'relative',flexShrink:0}}>
                <button type='button' onClick={()=>setAgentStatusDropOpen(o=>!o)}
                  ...
                  {agentStatusSet.length===0?'All Stages':`${agentStatusSet.length} selected`} Γû╛
                </button>
                {agentStatusDropOpen&&(
              ...
              <div style={{position:'relative',display:'inline-block'}}>
                <select value={filterLoanAmount} onChange={e=>setFilterLoanAmount(e.target.value)}
                  ...
                  <option value="0-100000">Up to Γé╣1 Lakh</option>
                  <option value="100000-300000">Γé╣1L - Γé╣3L</option>
                  <option value="300000-500000">Γé╣3L - Γé╣5L</option>
                  <option value="500000-1000000">Γé╣5L - Γé╣10L</option>
                  <option value="1000000-2000000">Γé╣10L - Γé╣20L</option>
                  <option value="2000000-99999999">Above Γé╣20L</option>
                </select>
                <span style={{position:'absolute',right:10,top:'50%',transform:'translateY(-50%)',color:'#6b7280',fontSize:9,pointerEvents:'none',fontWeight:700}}>Γû╝</span>
              </div>
              <div style={{position:'relative',display:'inline-block'}}>
            ...
            {isMobile?(
              <div>
                {loading?(
                  <div style={{padding:30,textAlign:'center',color:txt2}}>LoadingΓÇª</div>
                      ...
                      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10}}>
                        <div style={{display:'flex',alignItems:'center',gap:10}}>
                          ...
                          <div>
                            ...
                            {lead.sheet_number&&<div style={{fontSize:10,color:txt2,marginTop:1}}>≡ƒôä {lead.sheet_number}</div>}
                            ...
                            {(()=>{const obs=leadObligations[lead.id]||[];if(!obs.length)return null;const totalObl=obs.reduce((s,o)=>s+calculateObligatedEMI(o),0);const sal=parseFloat(lead.monthly_salary)||0;const foir=sal>0?Math.round((totalObl/sal)*100):null;return(<div style={{fontSize:10,color:'#6366f1',fontWeight:600,marginTop:1}}>≡ƒôï Obl. EMI: Γé╣{totalObl.toLocaleString('en-IN')}{foir!==null?` | FOIR: ${foir}%`:''}</div>)})()}
                            {lead.notes&&<div style={{fontSize:10,color:'#059669',fontWeight:600,marginTop:1}}>≡ƒô¥ Notes updated</div>}
                            {leadCallbackMap[lead.id]&&(
                              <div style={{fontSize:10,color:'#92400E',fontWeight:600,marginTop:1}}>
                                ≡ƒôà {fmtISTDate(leadCallbackMap[lead.id].due_date)} {fmtISTTime(leadCallbackMap[lead.id].due_date)}
                              </div>
                            )}
                          </div>
                        </div>
              ...
              <div className="mobile-table" style={{background:bg1,border:'1px solid '+bdr,borderRadius:12,overflow:'hidden'}}>
                ...
                <div style={{overflowX:'auto'}}>
                  <table style={{width:'100%',borderCollapse:'collapse'}}>
                    ...
                    <tbody>
                      {loading?(
                        <tr><td colSpan={6} style={{padding:40,textAlign:'center',fontSize:13,color:txt2}}>LoadingΓÇª</td></tr>
                      ):filteredLeads.length===0?(
                        ...
                        return(
                          <tr key={lead.id} style={{borderBottom:'1px solid '+bdr,transition:'background 0.1s'}}
                            ...
                            <td style={{padding:'12px 14px',width:240,maxWidth:240}}>
                              <div style={{display:'flex',alignItems:'center',gap:9}}>
                                ...
                                <div>
                                  ...
                                  {lead.call_count>0&&<div style={{fontSize:10,color:'#9CA3AF'}}>≡ƒô₧ {lead.call_count} calls</div>}
                                  {(()=>{const obs=leadObligations[lead.id]||[];if(!obs.length)return null;const totalObl=obs.reduce((s,o)=>s+calculateObligatedEMI(o),0);const sal=parseFloat(lead.monthly_salary)||0;const foir=sal>0?Math.round((totalObl/sal)*100):null;return(<div style={{fontSize:10,color:'#6366f1',fontWeight:600,marginTop:1}}>≡ƒôï Obl. EMI: Γé╣{totalObl.toLocaleString('en-IN')}{foir!==null?` | FOIR: ${foir}%`:''}</div>)})()}
                                  {lead.notes&&<div style={{fontSize:10,color:'#059669',fontWeight:600,marginTop:1}}>≡ƒô¥ Notes updated</div>}
                                  {leadCallbackMap[lead.id]&&(
                                    <div style={{fontSize:10,color:'#92400E',fontWeight:600,marginTop:1}}>
                                      ≡ƒôà Callback: {fmtISTDate(leadCallbackMap[lead.id].due_date)} {fmtISTTime(leadCallbackMap[lead.id].due_date)}
                                    </div>
                                  )}
                                </div>
                              </div>
                            ...
                            <td style={{padding:'12px 14px',display:'none'}}>
                              ...
                              <div style={{fontSize:10,color:txt2}}>{pIdx>=0&&pIdx<PIPELINE.length-1?`Next: ${PIPELINE[pIdx+1]}`:pIdx===PIPELINE.length-1?'Γ£à Final':''}</div>
                            </td>
        ...
        {/* ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ CALLS TAB ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ */}
        {activeTab==='calls'&&(
                        ...
                        <div key={call.id} style={{padding:'12px 0',borderBottom:'1px solid '+bdr}}>
                          ...
                          <div style={{display:'flex',flexWrap:'wrap',alignItems:'center',gap:8,fontSize:12,color:txt2}}>
                            ...
                            {lead && getLeadObligations(lead.id).length>0&&(
                              <span style={{background:'#EFF6FF',color:'#1D4ED8',padding:'3px 8px',borderRadius:20,fontSize:11,fontWeight:600}}>≡ƒº╛ {getLeadObligations(lead.id).length} obligations</span>
                            )}
                          </div>
                          ...
                          return(
                            <tr key={call.id} style={{borderBottom:'1px solid '+bdr}}>
                              <td style={{padding:'11px 14px',fontWeight:600,fontSize:13,color:txt1}}>
                                ...
                                {lead&&getLeadObligations(lead.id).length>0&&(
                                  <div style={{marginTop:4,fontSize:11,color:'#1D4ED8',fontWeight:600}}>≡ƒº╛ {getLeadObligations(lead.id).length} obligations</div>
                                )}
                              </td>
        ...
        {/* ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ TASKS TAB ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ */}
        {activeTab==='tasks'&&(
          <div>
                    ...
                    <div key={task.id} style={{background:bg1,border:'1px solid '+(od?'#FCA5A5':bdr),borderRadius:10,padding:'13px 14px',marginBottom:8,borderLeft:'3px solid '+(od?'#EF4444':task.priority==='High'?'#EF4444':task.priority==='Medium'?'#F59E0B':'#22C55E')}}>
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                        <div style={{flex:1,minWidth:0,marginRight:8}}>
                          <div style={{fontWeight:600,fontSize:13,color:od?'#DC2626':txt1,marginBottom:3}}>{od&&'ΓÜá∩╕Å '}{task.title}</div>
                          {task.notes&&<div style={{fontSize:12,color:txt2,marginBottom:3}}>≡ƒô¥ {task.notes}</div>}
                          <div style={{fontSize:11,color:od?'#DC2626':txt2,fontWeight:od?600:400}}>≡ƒòÉ {fmtIST(task.due_date)}</div>
                        </div>
      ...
      {/* ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ VIEW LEAD MODAL ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ */}
      {viewLead&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.45)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:16}} onClick={()=>setViewLead(null)}>
          <div style={{background:bg1,borderRadius:16,padding:24,maxWidth:440,width:'100%',boxShadow:'0 16px 48px rgba(0,0,0,0.2)'}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:18}}>
              ...
              <button onClick={()=>setViewLead(null)} style={{background:'none',border:'none',cursor:'pointer',fontSize:22,color:txt2,lineHeight:1}}>├ù</button>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
              {[
                ['Current Status',viewLead.status||'ΓÇö'],
                ['Previous Status',viewLead.previous_status||'ΓÇö'],
                ['Previous Agent',viewLead.previous_agent_name||'ΓÇö'],
                ...
                ['Sheet No.',viewLead.sheet_number||'ΓÇö'],
                ['City',viewLead.city||'ΓÇö'],
                ['Loan Amount',loanAmtDisplay(viewLead)],
                ['Company',viewLead.company||'ΓÇö'],
                ['App ID',viewLead.application_id||'ΓÇö'],
              ].map(([label,val])=>{
  ...
  return(
    <div>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16,flexWrap:'wrap',gap:8}}>
        <div>
          <div style={{fontWeight:600,fontSize:15,color:'#111827',marginBottom:2}}>≡ƒô▒ WhatsApp Templates</div>
        ...
        <button onClick={saveTemplates} disabled={loading}
          ...
          <IconDeviceFloppy size={15}/>{loading?'SavingΓÇª':saved?'Γ£à Saved!':'Save Templates'}
        </button>
      </div>
...
function TeamLeaderPanel({ userId }) {
  ...
  const checkOverdueTasks=async()=>{
    ...
    // No DB-side due_date filter ΓÇö tasks.due_date is `timestamptz`, and a
      ...
      <div className="page-body">
        ...
        {activeTab==='overview'&&(
          <div className="table-container">
            ...
            {loading?<div style={{padding:40,textAlign:'center',color:'#A0AEC0'}}>LoadingΓÇª</div>:(
              isMobile?(
                <div style={{padding:'8px 12px'}}>
                  {agentStats.map(a=>(
      ...
      <div className="page-body">
        ...
        {activeTab==='calls'&&(
          <div className="table-container">
            ...
            {loading?
              <div style={{padding:40,textAlign:'center',color:'#A0AEC0'}}>LoadingΓÇª</div>
            :calls.length===0?
...
// ΓöÇΓöÇΓöÇ BANK STATEMENT ANALYZER ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  return (
    <div style={{padding:24,background:'#f8fafc',minHeight:'100vh'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20}}>
        <div>
          <h2 style={{fontSize:18,fontWeight:700,color:'#111827',margin:0}}>≡ƒöì Bank Statement Analyzer</h2>
          <div style={{fontSize:13,color:'#6b7280',marginTop:4}}>Local credit risk analysis for loan processing</div>
        </div>
        {result && <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}><Btn onClick={downloadXLSX}>Γ¼ç Excel Report</Btn><Btn outline onClick={downloadCSV}>Γ¼ç CSV</Btn><SaveBsaToLead bsaResult={result} userId={userId} /></div>}
      </div>
      ...
      {!result && (
        <Card style={{padding:32,textAlign:'center',marginBottom:24}}>
          ...
          >
            <div style={{fontSize:40,marginBottom:12}}>≡ƒôä</div>
            ...
            {file && <div style={{marginTop:12,color:BSA_P,fontWeight:500,fontSize:13}}>Γ£ô {file.name}</div>}
          </div>
          ...
          {file && (
            <Btn onClick={analyze} disabled={loading} style={{marginTop:20,padding:'10px 32px',fontSize:14}}>
              {loading?'ΓÅ│ Analyzing...':'≡ƒöì Analyze Statement'}
            </Btn>
          )}
      ...
      {loading && (
        <Card style={{padding:48,textAlign:'center'}}>
          <div style={{fontSize:32,marginBottom:12}}>ΓÅ│</div>
      ...
      {result && !loading && (
        <div>
          <Btn outline small onClick={()=>{setResult(null);setFile(null)}} style={{marginBottom:20}}>ΓåÉ Analyze Another</Btn>
          ...
          <Card style={{padding:20,marginBottom:16,borderLeft:`4px solid ${riskColor(result.credit_assessment?.overall_risk)}`}}>
            <div style={{display:'flex',flexWrap:'wrap',gap:24,alignItems:'flex-start'}}>
              ...
              <div><div style={{fontSize:11,color:'#6b7280',fontWeight:600,marginBottom:4}}>EST. MONTHLY INCOME</div><div style={{fontSize:16,fontWeight:700,color:'#111827'}}>Γé╣{(result.credit_assessment?.estimated_monthly_income||0).toLocaleString('en-IN')}</div></div>
              <div><div style={{fontSize:11,color:'#6b7280',fontWeight:600,marginBottom:4}}>TOTAL EMI BURDEN</div><div style={{fontSize:16,fontWeight:700,color:'#d97706'}}>Γé╣{(result.credit_assessment?.total_emi_burden||0).toLocaleString('en-IN')}</div></div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(150px,1fr))',gap:10,marginBottom:16}}>
            {[
              {l:'Total Credits',      v:'Γé╣'+(result.summary?.total_credits||0).toLocaleString('en-IN'),  c:'#16a34a'},
              {l:'Total Debits',       v:'Γé╣'+(result.summary?.total_debits||0).toLocaleString('en-IN'),   c:'#dc2626'},
              {l:'Avg Monthly Balance',v:'Γé╣'+(result.summary?.average_monthly_balance||0).toLocaleString('en-IN'), c:BSA_P},
          ...
          {result.risk_flags?.length>0&&(
            <Card style={{marginBottom:16}}>
              <div style={{padding:'14px 20px',borderBottom:'1px solid #e5e7eb',fontWeight:600,fontSize:14,color:'#dc2626'}}>≡ƒö┤ Risk Flags ({result.risk_flags.length})</div>
              <div style={{overflowX:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  ...
                  <tbody>{result.risk_flags.map((f,i)=>(
                    <tr key={i} style={{background:sevColor(f.severity)}}>
                      ...
                      <td style={{...TD,fontWeight:600}}>Γé╣{(f.amount||0).toLocaleString('en-IN')}</td>
          ...
          {result.cc_vendor_funding?.length>0&&(
            <Card style={{marginBottom:16}}>
              <div style={{padding:'14px 20px',borderBottom:'1px solid #e5e7eb',fontWeight:600,fontSize:14,color:'#7c3aed'}}>≡ƒÆ│ Credit Card / Fintech Vendor Funding ({result.cc_vendor_funding.length})</div>
              <div style={{overflowX:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  ...
                  <tbody>{result.cc_vendor_funding.map((f,i)=>(
                    <tr key={i} style={{background:'#faf5ff'}}>
                      ...
                      <td style={TD}>{f.date}</td><td style={TD}>{f.description}</td>
                      <td style={{...TD,fontWeight:600,color:'#7c3aed'}}>Γé╣{(f.amount||0).toLocaleString('en-IN')}</td>
                    </tr>
                  ))}</tbody>
                </table>
              </div>
          ...
          {result.emi_obligations?.length>0&&(
            <Card style={{marginBottom:16}}>
              <div style={{padding:'14px 20px',borderBottom:'1px solid #e5e7eb',fontWeight:600,fontSize:14,color:'#d97706'}}>≡ƒôï EMI & Recurring Obligations</div>
              <div style={{overflowX:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  ...
                  <tbody>{result.emi_obligations.map((e,i)=>(
                    <tr key={i} style={{background:'#fffbeb'}}>
                      ...
                      <td style={{...TD,fontWeight:700,color:'#d97706'}}>Γé╣{(e.amount||0).toLocaleString('en-IN')}</td>
          ...
          {result.watchlist?.length>0&&(
            <Card style={{marginBottom:16}}>
              <div style={{padding:'14px 20px',borderBottom:'1px solid #e5e7eb',fontWeight:600,fontSize:14,color:'#d97706'}}>≡ƒƒí Watchlist ({result.watchlist.length})</div>
              <div style={{overflowX:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  ...
                  <tbody>{result.watchlist.map((w,i)=>(
                    <tr key={i} style={{background:'#fffbeb'}}>
                      ...
                      <td style={{...TD,fontWeight:600}}>Γé╣{(w.amount||0).toLocaleString('en-IN')}</td>
                    </tr>
                  ))}</tbody>
                </table>
              </div>
          ...
          {result.positive_signals?.length>0&&(
            <Card style={{marginBottom:16}}>
              <div style={{padding:'14px 20px',borderBottom:'1px solid #e5e7eb',fontWeight:600,fontSize:14,color:'#16a34a'}}>≡ƒƒó Positive Signals</div>
              <div style={{overflowX:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  <thead><tr>{['Type','Date','Description','Amount'].map(h=><th key={h} style={TH}>{h}</th>)}</tr></thead>
                  <tbody>{result.positive_signals.map((s,i)=>(
                    <tr key={i} style={{background:'#f0fdf4'}}>
                      ...
                      <td style={{...TD,fontWeight:600,color:'#16a34a'}}>Γé╣{(s.amount||0).toLocaleString('en-IN')}</td>
                    </tr>
                  ))}</tbody>
                </table>
              </div>
          ...
          {result.repeat_parties?.length>0&&(
            <Card style={{marginBottom:16}}>
              <div style={{padding:'14px 20px',borderBottom:'1px solid #e5e7eb',fontWeight:600,fontSize:14}}>≡ƒæÑ Repeat Transaction Parties</div>
              <div style={{overflowX:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  ...
                  <tbody>{result.repeat_parties.map((p,i)=>(
                    <tr key={i} style={{background:p.flag==='SUSPICIOUS'?'#fef2f2':'white'}}>
                      ...
                      <td style={{...TD,color:'#dc2626'}}>Γé╣{(p.total_debit||0).toLocaleString('en-IN')}</td>
                      <td style={{...TD,color:'#16a34a'}}>Γé╣{(p.total_credit||0).toLocaleString('en-IN')}</td>
          ...
          {result.monthly_cashflow?.length>0&&(
            <Card style={{marginBottom:16}}>
              <div style={{padding:'14px 20px',borderBottom:'1px solid #e5e7eb',fontWeight:600,fontSize:14}}>≡ƒôê Month-wise Cash Flow</div>
              <div style={{overflowX:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  ...
                  <tbody>{result.monthly_cashflow.map((m,i)=>(
                    <tr key={i} style={{background:m.bounce_count>0?'#fef2f2':'white'}}>
                      ...
                      <td style={{...TD,color:'#16a34a',fontWeight:600}}>Γé╣{(m.total_credit||0).toLocaleString('en-IN')}</td>
                      <td style={{...TD,color:'#dc2626',fontWeight:600}}>Γé╣{(m.total_debit||0).toLocaleString('en-IN')}</td>
                      <td style={{...TD,fontWeight:600}}>Γé╣{(m.closing_balance||0).toLocaleString('en-IN')}</td>
          ...
          {result.ecs_returns?.length>0&&(
            <Card style={{marginBottom:16}}>
              <div style={{padding:'14px 20px',borderBottom:'1px solid #fecaca',fontWeight:600,fontSize:14,color:'#dc2626',background:'#fef2f2'}}>Γå⌐∩╕Å ECS / NACH Returns ({result.ecs_returns.length})</div>
              <div style={{overflowX:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  ...
                  <tbody>{result.ecs_returns.map((r,i)=>(
                    <tr key={i} style={{background:'#fef2f2'}}>
                      ...
                      <td style={{...TD,fontWeight:600,color:'#dc2626'}}>Γé╣{(r.return_amount||0).toLocaleString('en-IN')}</td>
                      <td style={{...TD,whiteSpace:'nowrap'}}>{r.charge_date}</td>
                      <td style={{...TD,fontWeight:600,color:'#d97706'}}>Γé╣{(r.charge_amount||0).toLocaleString('en-IN')}</td>
          ...
          {/* CC Card Rotation ΓÇö high severity RED */}
          {result.cc_card_rotation?.detected&&result.cc_card_rotation?.transactions?.length>0&&(
            <Card style={{marginBottom:16,borderLeft:'4px solid #dc2626'}}>
              <div style={{padding:'14px 20px',borderBottom:'1px solid #fecaca',fontWeight:600,fontSize:14,color:'#dc2626',background:'#fef2f2'}}>
                ≡ƒö┤ CC Card Rotation / Cash Advance ({result.cc_card_rotation.transaction_count} transactions ┬╖ Γé╣{(result.cc_card_rotation.total_amount||0).toLocaleString('en-IN')} total)
              </div>
              <div style={{overflowX:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  ...
                  <tbody>{result.cc_card_rotation.transactions.map((t,i)=>(
                    <tr key={i} style={{background:'#fef2f2'}}>
                      ...
                      <td style={{...TD,fontWeight:600,color:'#dc2626'}}>Γé╣{(t.amount||0).toLocaleString('en-IN')}</td>
          ...
          {result.stock_market_activity?.detected&&result.stock_market_activity?.transactions?.length>0&&(
            <Card style={{marginBottom:16}}>
              <div style={{padding:'14px 20px',borderBottom:'1px solid #e5e7eb',fontWeight:600,fontSize:14,color:'#7c3aed'}}>
                ≡ƒôê Stock Market / Trading Activity ({result.stock_market_activity.transaction_count} txns ┬╖ Invested Γé╣{(result.stock_market_activity.total_invested||0).toLocaleString('en-IN')} ┬╖ Withdrawn Γé╣{(result.stock_market_activity.total_withdrawn||0).toLocaleString('en-IN')})
              </div>
                ...
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  ...
                  <tbody>{result.stock_market_activity.transactions.map((t,i)=>(
                    <tr key={i} style={{background:'#faf5ff'}}>
                      ...
                      <td style={{...TD,whiteSpace:'nowrap'}}>{t.date}</td>
                      <td style={{...TD,fontWeight:600,color:'#7c3aed'}}>Γé╣{(t.amount||0).toLocaleString('en-IN')}</td>
            ...
            <Card style={{marginBottom:16,borderLeft:`4px solid ${result.small_loan_disbursals.frequent?'#dc2626':'#d97706'}`}}>
              <div style={{padding:'14px 20px',borderBottom:'1px solid #e5e7eb',display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}}>
                <span style={{fontWeight:600,fontSize:14,color:result.small_loan_disbursals.frequent?'#dc2626':'#d97706'}}>
                  ≡ƒÆ╕ Small Loan Disbursals ({result.small_loan_disbursals.disbursal_count} ┬╖ Γé╣{(result.small_loan_disbursals.total_disbursed||0).toLocaleString('en-IN')} total)
                </span>
                {result.small_loan_disbursals.frequent&&<Badge text='FREQUENT ΓÇö HIGH RISK' color='#dc2626'/>}
              </div>
                ...
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  ...
                  <tbody>{result.small_loan_disbursals.disbursals.map((d,i)=>(
                    <tr key={i} style={{background:result.small_loan_disbursals.frequent?'#fef2f2':'#fffbeb'}}>
                      ...
                      <td style={{...TD,fontWeight:600,color:result.small_loan_disbursals.frequent?'#dc2626':'#d97706'}}>Γé╣{(d.amount||0).toLocaleString('en-IN')}</td>
          ...
          {result.wallet_to_bank?.length>0&&(
            <Card style={{marginBottom:16}}>
              <div style={{padding:'14px 20px',borderBottom:'1px solid #e5e7eb',fontWeight:600,fontSize:14,color:'#0891b2'}}>≡ƒæ¢ Wallet-to-Bank Transfers ({result.wallet_to_bank.length})</div>
              <div style={{overflowX:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  ...
                  <tbody>{result.wallet_to_bank.map((w,i)=>(
                    <tr key={i} style={{background:'#ecfeff'}}>
                      ...
                      <td style={{...TD,fontWeight:600,color:'#0891b2'}}>Γé╣{(w.amount||0).toLocaleString('en-IN')}</td>
          ...
          {result.frequent_transfers?.length>0&&(
            <Card style={{marginBottom:16}}>
              <div style={{padding:'14px 20px',borderBottom:'1px solid #e5e7eb',fontWeight:600,fontSize:14}}>≡ƒöä Frequent Transfers ({result.frequent_transfers.length})</div>
              <div style={{overflowX:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  ...
                  <tbody>{result.frequent_transfers.map((t,i)=>(
                    <tr key={i} style={{background:t.is_self?'#eff6ff':'white'}}>
                      ...
                      <td style={{...TD,fontWeight:600}}>Γé╣{(t.total_amount||0).toLocaleString('en-IN')}</td>
          ...
          {/* Forex / International Trading ΓÇö high severity RED */}
          {result.forex_trading?.length>0&&(
            <Card style={{marginBottom:16,borderLeft:'4px solid #dc2626'}}>
              <div style={{padding:'14px 20px',borderBottom:'1px solid #fecaca',fontWeight:600,fontSize:14,color:'#dc2626',background:'#fef2f2'}}>≡ƒîÉ Forex / International Trading ({result.forex_trading.length})</div>
              <div style={{overflowX:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  ...
                  <tbody>{result.forex_trading.map((f,i)=>(
                    <tr key={i} style={{background:'#fef2f2'}}>
                      ...
                      <td style={{...TD,fontWeight:600,color:'#dc2626'}}>Γé╣{(f.amount||0).toLocaleString('en-IN')}</td>
          ...
          {result.all_transactions?.length>0&&(
            <Card style={{marginBottom:16}}>
              <div style={{padding:'14px 20px',borderBottom:'1px solid #e5e7eb',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                <div style={{fontWeight:600,fontSize:14}}>≡ƒôè All Transactions ({result.all_transactions.length})</div>
                <Btn small onClick={downloadCSV}>Γ¼ç CSV</Btn>
              </div>
              <div style={{overflowX:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  ...
                  <tbody>{result.all_transactions.map((t,i)=>(
                    <tr key={i} style={{background:catColor(t.category)}}>
                      ...
                      <td style={{...TD,color:'#dc2626',fontWeight:t.debit?600:400}}>{t.debit?'Γé╣'+t.debit.toLocaleString('en-IN'):'ΓÇö'}</td>
                      <td style={{...TD,color:'#16a34a',fontWeight:t.credit?600:400}}>{t.credit?'Γé╣'+t.credit.toLocaleString('en-IN'):'ΓÇö'}</td>
                      <td style={TD}>{t.balance?'Γé╣'+t.balance.toLocaleString('en-IN'):'ΓÇö'}</td>
                      ...
                      <td style={TD}>{t.flag?<Badge text={t.flag} color='#dc2626'/>:'ΓÇö'}</td>
                    </tr>
                  ))}</tbody>
                </table>
              </div>
...
export default function Dashboard({ session }) {
  ...
  const navSections=[
    ...
    {section:'TOOLS',items:[
      ...
      {id:'bsa',               label:'≡ƒöì Bank Analyzer',        icon:'chart-bar',roles:['agent','team_leader','manager','admin']},
      ...
      {id:'cam',               label:'≡ƒôï CAM Builder',          icon:'chart-bar',roles:['agent','team_leader','manager','admin']},
    ]},
  ...
  const AdminPanel=()=>{
    ...
    const fmtAmt=n=>{ const v=Number(n)||0; if(v>=10000000) return 'Γé╣'+(v/10000000).toFixed(v%10000000===0?0:1)+'Cr'; if(v>=100000) return 'Γé╣'+(v/100000).toFixed(v%100000===0?0:1)+'L'; return 'Γé╣'+v.toLocaleString('en-IN') }
    const amtLabel=leadAmtMin===''&&leadAmtMax===''?'All Amounts':leadAmtMin&&leadAmtMax?`${fmtAmt(leadAmtMin)}ΓÇô${fmtAmt(leadAmtMax)}`:leadAmtMin?`${fmtAmt(leadAmtMin)}+`:`Up to ${fmtAmt(leadAmtMax)}`
    ...
    //    completely untouched ΓÇö the original agent keeps working it with
    ...
    //    access AND the new agent also gets it via the same shared row ΓÇö
    ...
    const doReassignLead=async()=>{
      ...
      try{
        ...
        showApToast(obligationsFailed
          ? ('Lead shared with '+agentName+' ΓÇö obligations couldn\'t be copied, please check manually')
          : (reassignType==='mirror'
    ...
    const doReassignExport=async(leadsToExport)=>{
      ...
      showApToast('Preparing exportΓÇª')
      try{
        ...
        const rows=leadsToExport.map(l=>{
          ...
          const oblSummary=obls.map(o=>`${o.obligation_type||'Loan'} @ ${o.bank_name||'ΓÇö'} EMI Γé╣${o.emi_amount||0}`).join(' | ')
    ...
    const resetPassword=async()=>{
      ...
      try{
        ...
        if(!error&&data?.user?.id) alert('Γ£à Password reset to Capital@123 for '+resetEmail)
        else alert('Error: '+(error?.message||JSON.stringify(data)))
      }catch(err){alert('Error: '+err.message)}
            ...
            <div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))',gap:10,marginBottom:16}}>
                {manualStats.map(m=>(<div key={m.key} style={{background:'white',padding:'14px 16px',borderRadius:10,border:'1px solid #E2E8F0'}}>
                  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:4}}><div style={{fontSize:12,color:'#718096',fontWeight:500}}>{m.label}</div><button onClick={()=>{setEditManualKey(m.key);setEditManualVal(settings[m.key]||'')}} style={{background:'none',border:'none',cursor:'pointer',color:'#A0AEC0',fontSize:13,padding:0}}>Γ£Å</button></div>
                  {editManualKey===m.key?(<div style={{display:'flex',gap:6}}><input className='form-input' style={{flex:1,padding:'4px 8px',fontSize:13}} value={editManualVal} onChange={e=>setEditManualVal(e.target.value)} autoFocus onKeyDown={e=>{if(e.key==='Enter')saveManualSetting(m.key,editManualVal)}}/><button className='btn btn-primary btn-sm' onClick={()=>saveManualSetting(m.key,editManualVal)} disabled={savingManual}>{savingManual?'ΓÇª':'Save'}</button></div>):(<div style={{fontSize:24,fontWeight:700,color:'#185FA5'}}>{settings[m.key]||'ΓÇö'}</div>)}
                </div>))}
              </div>
              <div className='table-container' style={{marginBottom:16}}>
                <div style={{padding:'12px 16px',borderBottom:'1px solid #E2E8F0',fontWeight:600,fontSize:14}}>Agent Performance ΓÇö {new Date().toLocaleDateString('en-IN',{month:'long',year:'numeric',timeZone:IST_TZ})}</div>
            ...
            <div>
              ...
              {showUserForm&&(
                <div className="card" style={{marginBottom:16}}>
                  <div className="card-header"><h3>Add New Member</h3><button className="btn btn-ghost btn-sm" onClick={()=>setShowUserForm(false)}>Γ£ò</button></div>
                  <div className="card-body">
                    <form onSubmit={createUser}>
                      ...
                      <div style={{background:'#FFFAF0',border:'1px solid #F6E05E',borderRadius:8,padding:'10px 12px',marginBottom:12,fontSize:13,color:'#744210'}}>ΓÜá∩╕Å Default password: <strong>Capital@123</strong></div>
                      ...
                      :filteredUsers.map((user,i)=>{
                        ...
                        return(
                          <tr key={user.id}>
                            ...
                            <td style={{fontSize:12,color:'#4A5568'}}>{user.mobile||'ΓÇö'}</td>
                            ...
                            <td>
                              <div style={{display:'flex',gap:5,alignItems:'center'}}>
                                ...
                                <button className="btn btn-ghost btn-sm" onClick={()=>toggleUserStatus(user.id,user.status||'active')}>{user.status==='active'?'Deactivate':'Activate'}</button>
                                <button onClick={()=>deleteUser(user.id,user.email)} style={{width:28,height:28,borderRadius:6,border:'1px solid #FED7D7',background:'transparent',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',color:'#FC8181',fontSize:16}}>├ù</button>
                              </div>
                            </td>
                          </tr>
                        )
                  ...
                  <div key={tl.id} className="card" style={{marginBottom:12,overflow:'hidden'}}>
                    <div style={{background:'linear-gradient(135deg,#0C3A6B,#185FA5)',padding:'11px 16px',color:'white',fontWeight:600,fontSize:13,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                      <div style={{display:'flex',alignItems:'center',gap:8}}>
                        ...
                        {tl.full_name}<span style={{opacity:0.7,fontWeight:400,fontSize:11}}>ΓÇö Team Leader</span>
                      </div>
          ...
          {activeTab==='leaderboard'&&(
            <div>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14}}>
                <h3 style={{fontSize:15,fontWeight:600}}>Leaderboard ΓÇö Calls This Week</h3>
                    ...
                    <tbody>
                      {leaderboard.map((row,i)=>(
                        <tr key={row.agentId}>
                          ...
                          <td>
                            {i===0&&<span style={{background:'#FEF3C7',color:'#92400E',padding:'2px 8px',borderRadius:4,fontSize:11,fontWeight:600}}>≡ƒÑç Top</span>}
                            {i===1&&<span style={{background:'#F3F4F6',color:'#374151',padding:'2px 8px',borderRadius:4,fontSize:11,fontWeight:600}}>≡ƒÑê 2nd</span>}
          ...
          {activeTab==='leads_admin'&&(
            <div>
              <div style={{display:'flex',gap:8,marginBottom:12,flexWrap:'wrap',alignItems:'center'}}>
                ...
                <div style={{position:'relative'}}>
                  <button type='button' onClick={()=>setStatusDropOpen(o=>!o)} className='form-input' style={{width:'auto',fontSize:13,cursor:'pointer'}}>{leadStatusSet.length===0?'All Status':`${leadStatusSet.length} selected`} Γû╛</button>
                  {statusDropOpen&&(
                ...
                <div style={{position:'relative'}} ref={sheetDropRef}>
                  <button type='button' onClick={()=>setSheetDropOpen(o=>!o)} className='form-input' style={{width:'auto',fontSize:13,cursor:'pointer',borderColor:sheetDropOpen?'#185FA5':undefined}}>{selectedSheets.length===0?'Sheet No.':`Sheet No. (${selectedSheets.length})`} Γû╛</button>
                  {sheetDropOpen&&(
                ...
                <div style={{position:'relative'}} ref={amtDropRef}>
                  <button type='button' onClick={()=>setAmtDropOpen(o=>!o)} className='form-input' style={{width:'auto',fontSize:13,cursor:'pointer',borderColor:amtDropOpen?'#185FA5':undefined}}>{amtLabel} Γû╛</button>
                  {amtDropOpen&&(
                    <div style={{position:'absolute',top:'110%',left:0,zIndex:50,background:'white',border:'1px solid #E2E8F0',borderRadius:8,boxShadow:'0 8px 24px rgba(0,0,0,0.12)',padding:8,minWidth:200}}>
                      ...
                      <div style={{display:'flex',gap:6}}>
                        <input autoFocus type='number' className='form-input' style={{width:90,fontSize:13}} placeholder='Min Γé╣' value={leadAmtMin} onChange={e=>setLeadAmtMin(e.target.value)}/>
                        <input type='number' className='form-input' style={{width:90,fontSize:13}} placeholder='Max Γé╣' value={leadAmtMax} onChange={e=>setLeadAmtMax(e.target.value)}/>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <button className='btn btn-outline btn-sm' style={{whiteSpace:'nowrap',fontSize:12}} onClick={()=>doReassignExport(selected.size>0?adminLeads.filter(l=>selected.has(l.id)):filteredLeads)}>Γåô Export{selected.size>0?' Selected':''} for Reassignment</button>
                <button onClick={()=>setShowDupOnly(o=>!o)} style={{padding:'7px 13px',borderRadius:8,border:'1.5px solid '+(showDupOnly?'#dc2626':'#E2E8F0'),background:showDupOnly?'#FEF2F2':'white',color:showDupOnly?'#dc2626':'#64748B',fontSize:12,fontWeight:600,cursor:'pointer',whiteSpace:'nowrap'}}>ΓÜá∩╕Å Duplicates{dupLeadIds.size>0?' ('+dupLeadIds.size+')':''}</button>
              </div>
              {dupLeadIds.size>0&&(
                <div style={{fontSize:12,color:'#dc2626',fontWeight:600,marginBottom:10}}>ΓÜá∩╕Å {dupLeadIds.size} duplicate lead{dupLeadIds.size>1?'s':''} detected (same mobile assigned to the same agent)</div>
              )}
              {selected.size>0&&(
              ...
              <div className='table-container'>
                <div style={{padding:'10px 14px',borderBottom:'1px solid #E2E8F0',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:10}}>
                    ...
                    <button className='btn btn-ghost btn-sm' style={{fontSize:11}} onClick={fetchLeads}>Γå╗ Refresh</button>
                  </div>
                </div>
...
<td style={{padding:'8px 6px',verticalAlign:'middle',overflow:'hidden'}}><div style={{fontWeight:600,fontSize:12,color:'#1E293B',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}} title={l.full_name||''}>{l.full_name||'ΓÇö'}</div>{isDup&&<span style={{display:'inline-block',marginTop:2,padding:'1px 5px',borderRadius:8,background:'#FEF3C7',color:'#92400E',fontSize:9,fontWeight:700}}>ΓÜá∩╕Å DUP</span>}</td>
<td style={{padding:'8px 6px',verticalAlign:'middle',fontSize:11.5,color:'#475569',fontVariantNumeric:'tabular-nums',whiteSpace:'nowrap'}}>{l.mobile||'ΓÇö'}</td>
...
<td style={{padding:'8px 6px',verticalAlign:'middle',fontSize:11.5,color:'#1E293B',fontWeight:500,whiteSpace:'nowrap'}}>{l.loan_amount?'Γé╣'+Number(l.loan_amount).toLocaleString('en-IN'):<span style={{color:'#CBD5E1'}}>ΓÇö</span>}</td>


<td style={{padding:'8px 4px',verticalAlign:'middle',whiteSpace:'nowrap'}}>
  <div style={{display:'flex',alignItems:'center',gap:3}}>
    <button onClick={()=>setViewLead(l)} title='View lead' style={{display:'inline-flex',alignItems:'center',justifyContent:'center',width:26,height:26,borderRadius:6,background:'white',border:'1px solid #CBD5E1',color:'#185FA5',fontSize:13,cursor:'pointer'}}>≡ƒæü</button>
    ...
    <button onClick={()=>{setReassignTarget(l);setReassignTo('');setReassignType('transfer')}} title='Reassign lead' style={{display:'inline-flex',alignItems:'center',justifyContent:'center',width:26,height:26,borderRadius:6,background:'#EFF6FF',border:'1px solid #BFDBFE',color:'#1D4ED8',fontSize:13,cursor:'pointer'}}>Γå⌐</button>
    <button onClick={()=>handleUnassignLead(l)} title='Unassign agent' style={{display:'inline-flex',alignItems:'center',justifyContent:'center',width:26,height:26,borderRadius:6,background:'#FFF7ED',border:'1px solid #FED7AA',color:'#C2410C',fontSize:13,cursor:'pointer'}}>Γèÿ</button>
    <button onClick={()=>handleDeleteLead(l)} title='Delete lead permanently' style={{display:'inline-flex',alignItems:'center',justifyContent:'center',width:26,height:26,borderRadius:6,background:'#FEF2F2',border:'1px solid #FECACA',color:'#DC2626',fontSize:12,cursor:'pointer'}}>≡ƒùæ</button>
  </div>
</td>
<td style={{padding:'8px 6px',verticalAlign:'middle',fontSize:11,color:'#94A3B8',whiteSpace:'nowrap'}}>{l.created_at?new Date(l.created_at).toLocaleDateString('en-IN',{timeZone:IST_TZ}):'ΓÇö'}</td>
</tr>) })}
                  </tbody></table>
                </div>
              </div>
                ...
                <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.55)',zIndex:3000,display:'flex',alignItems:'center',justifyContent:'center',padding:16}} onClick={()=>{setReassignTarget(null);setReassignTo('');setReassignType('transfer')}}>
                  <div onClick={e=>e.stopPropagation()} style={{background:'white',borderRadius:16,width:'100%',maxWidth:440,padding:28,boxShadow:'0 20px 60px rgba(0,0,0,0.3)'}}>
                    ...
                    <div style={{display:'flex',gap:8,marginBottom:12}}>
                      <button onClick={()=>setReassignType('transfer')} style={{flex:1,padding:'8px 10px',borderRadius:8,border:reassignType==='transfer'?'2px solid #185FA5':'1px solid #E2E8F0',background:reassignType==='transfer'?'#EFF6FF':'white',fontSize:12,fontWeight:600,color:reassignType==='transfer'?'#185FA5':'#64748B',cursor:'pointer'}}>Γå⌐ Reassign</button>
                      <button onClick={()=>setReassignType('mirror')} style={{flex:1,padding:'8px 10px',borderRadius:8,border:reassignType==='mirror'?'2px solid #185FA5':'1px solid #E2E8F0',background:reassignType==='mirror'?'#EFF6FF':'white',fontSize:12,fontWeight:600,color:reassignType==='mirror'?'#185FA5':'#64748B',cursor:'pointer'}}>≡ƒ¬₧ Mirror</button>
                    </div>
                    {reassignType==='mirror'
                      ? <div style={{background:'#EFF6FF',border:'1px solid #BFDBFE',borderRadius:10,padding:'10px 14px',marginBottom:16,fontSize:12,color:'#1E40AF'}}>Γ£à <strong>Mirror:</strong> The original agent keeps access to this lead. The new agent also gets it with full history.</div>
                      : <div style={{background:'#F0FDF4',border:'1px solid #BBF7D0',borderRadius:10,padding:'10px 14px',marginBottom:16,fontSize:12,color:'#166534'}}>Γ£à <strong>Reassign:</strong> The original agent's lead is left completely untouched. The new agent gets their own independent copy as a fresh "New" lead ΓÇö nothing is taken away from anyone.</div>
                    }
                    ...
                    <select className='form-input' style={{width:'100%',marginBottom:20}} value={reassignTo} onChange={e=>setReassignTo(e.target.value)}>
                      <option value=''>Select agentΓÇª</option>
                      {users.filter(u=>['agent','team_leader'].includes(u.role)).map(u=><option key={u.id} value={u.id}>{u.full_name}</option>)}
                    </select>
                    <div style={{display:'flex',gap:10,justifyContent:'flex-end'}}>
                      ...
                      <button onClick={doReassignLead} disabled={reassigning||!reassignTo} style={{padding:'9px 20px',borderRadius:8,border:'none',background:reassignTo?'#185FA5':'#9CA3AF',color:'white',cursor:reassignTo?'pointer':'not-allowed',fontSize:13,fontWeight:600,opacity:reassigning?0.6:1}}>{reassigning?'WorkingΓÇª':(reassignType==='mirror'?'Mirror Lead':'Reassign Lead')}</button>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div>
              ...
              <div className='table-container'>
                <div style={{overflowX:'auto'}}>
                  ...
                  <tbody>{filteredAct.length===0?(<tr><td colSpan={6}><div className='empty-state'><h3>No activity yet</h3><p>Lead assignments will appear here</p></div></td></tr>):filteredAct.map(a=>(<tr key={a.id}><td style={{fontSize:11,color:'#718096',whiteSpace:'nowrap'}}>{a.created_at?new Date(a.created_at).toLocaleString('en-IN',{dateStyle:'short',timeStyle:'short',timeZone:IST_TZ}):'ΓÇö'}</td><td style={{fontWeight:500,fontSize:13}}>{a.lead_name||'ΓÇö'}</td><td><span style={{background:a.action==='Reassigned'?'#FEF3C7':'#E6F1FB',color:a.action==='Reassigned'?'#92400E':'#185FA5',padding:'2px 8px',borderRadius:4,fontSize:11,fontWeight:500}}>{a.action||'ΓÇö'}</span></td><td style={{fontSize:12}}>{a.assigned_to_name||users.find(u=>u.id===a.assigned_to)?.full_name||'ΓÇö'}</td><td style={{fontSize:12,color:'#718096'}}>{a.assigned_by_name||users.find(u=>u.id===a.assigned_by)?.full_name||'ΓÇö'}</td><td style={{fontSize:12,color:'#A0AEC0'}}>{a.previous_agent_name||'ΓÇö'}</td></tr>))}
                  </tbody></table>
                </div>
              </div>
            </div>
                ...
                {dispositions.map(d=>(
                  <div key={d.id} style={{background:'white',padding:'11px 13px',borderRadius:10,border:'1px solid #E2E8F0',display:'flex',alignItems:'center',justifyContent:'space-between',opacity:d.is_active?1:0.5}}>
                    ...
                    <div style={{display:'flex',gap:4}}>
                      ...
                      <button onClick={()=>deleteDisposition(d.id)} style={{width:22,height:22,border:'1px solid #FED7D7',borderRadius:4,background:'transparent',cursor:'pointer',color:'#FC8181',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14}}>├ù</button>
                    </div>
                  </div>
                ))}
              ...
              <div style={{display:'flex',flexDirection:'column',gap:6}}>
                {adminStages.map((s,i)=>(
                  <div key={s.id} style={{background:'white',padding:'10px 12px',borderRadius:10,border:'1px solid #E2E8F0',display:'flex',alignItems:'center',gap:8,opacity:s.is_active?1:0.55}}>
                    ...
                    <button onClick={()=>moveAdminStage(s.id,-1)} disabled={i===0} style={{background:'none',border:'none',cursor:i===0?'default':'pointer',color:i===0?'#E2E8F0':'#718096',fontSize:13,padding:'0 2px',lineHeight:1}}>Γû▓</button>
                    <button onClick={()=>moveAdminStage(s.id,1)} disabled={i===adminStages.length-1} style={{background:'none',border:'none',cursor:i===adminStages.length-1?'default':'pointer',color:i===adminStages.length-1?'#E2E8F0':'#718096',fontSize:13,padding:'0 2px',lineHeight:1}}>Γû╝</button>
                    ...
                    <button onClick={()=>deleteAdminStage(s.id,s.name)} style={{width:22,height:22,border:'1px solid #FED7D7',borderRadius:4,background:'transparent',cursor:'pointer',color:'#FC8181',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14}}>├ù</button>
                  </div>
                ))}
                ...
                {leadSources.map(s=>(
                  <div key={s.id} style={{background:'white',padding:'11px 13px',borderRadius:10,border:'1px solid #E2E8F0',display:'flex',alignItems:'center',justifyContent:'space-between',opacity:s.is_active?1:0.5}}>
                    ...
                    <div style={{display:'flex',gap:4}}>
                      ...
                      <button onClick={()=>deleteLeadSource(s.id)} style={{width:22,height:22,border:'1px solid #FED7D7',borderRadius:4,background:'transparent',cursor:'pointer',color:'#FC8181',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14}}>├ù</button>
                    </div>
                  </div>
                ))}
                ...
                <div className="card-body" style={{padding:0,maxHeight:300,overflowY:'auto'}}>
                  {activityLogs.length===0?(
                    ...
                    <div key={log.id} style={{padding:'10px 16px',borderBottom:'1px solid #F3F4F6'}}>
                      <div style={{fontSize:12,fontWeight:500,color:'#2D3748'}}>{log.profiles?.full_name||'Unknown'} ΓÇö <span style={{color:'#185FA5'}}>{log.action}</span></div>
  ...
  const AdminDashboardHome=()=>{
      ...
      try{
        ...
        const rows=toNotify.map(l=>{
          ...
          return{type:'stale_lead',lead_id:l.id,agent_id:l.assigned_to||null,agent_name:agentName,customer_name:l.full_name,stage:l.status,
            message:`≡ƒö┤ ${agentName}'s lead "${l.full_name}" has been in "${l.status}" for 3+ days with no Login.`}
        })
    ...
    const notifyAgentAssigned=async(agentId,count)=>{
      ...
      try{
        ...
        //   message:`≡ƒôÑ ${count} new lead${count>1?'s':''} assigned to you`,
        // }])
    ...
    const handleImport=async()=>{
      ...
      for(let i=0;i<valid.length;i+=100){
        ...
        else if(!data||data.length===0){ fail+=chunk.length; if(!firstError)firstError='No rows were inserted ΓÇö likely a Row Level Security (RLS) block on assigning leads to this agent.' }
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p>Welcome, {profile?.full_name||'Admin'} ΓÇö {new Date().toLocaleDateString('en-IN',{weekday:'short',day:'numeric',month:'short',timeZone:IST_TZ})}</p>
        </div>
          ...
          <div style={{position:'fixed',top:isMobile?0:'50%',left:isMobile?0:'50%',transform:isMobile?'none':'translate(-50%,-50%)',background:'white',borderRadius:isMobile?0:16,zIndex:600,width:isMobile?'100%':'94%',maxWidth:700,height:isMobile?'100%':'auto',maxHeight:isMobile?'100vh':'90vh',overflow:'hidden',display:'flex',flexDirection:'column',boxShadow:'0 32px 80px rgba(0,0,0,0.25)'}}>
            <div style={{background:'linear-gradient(135deg,#185FA5,#1e40af)',padding:'16px 20px',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
              <div>
                ...
                <div style={{color:'rgba(255,255,255,0.7)',fontSize:12,marginTop:2}}>Upload any Excel or CSV ΓÇö map columns in the next step</div>
              </div>
            ...
            <div style={{padding:20,overflowY:'auto',flex:1}}>
              ...
              <div style={{marginBottom:18}}>
                <div style={{fontSize:13,fontWeight:700,color:'#111827',marginBottom:8}}>Step 1 ΓÇö Upload Excel (.xlsx/.xls) or CSV file</div>
                <div style={{background:'#F8FAFC',border:'2px dashed #CBD5E0',borderRadius:10,padding:'20px',textAlign:'center',cursor:'pointer',position:'relative'}}
                  ...
                  <div style={{fontSize:11,color:'#9CA3AF',marginTop:4}}>Upload Excel (.xlsx/.xls) or CSV ΓÇö any column headers</div>
                </div>
              ...
              {fileHeaders.length>0&&(
                <div style={{marginBottom:18}}>
                  <div style={{fontSize:13,fontWeight:700,color:'#111827',marginBottom:8}}>Step 2 ΓÇö Map Columns <span style={{fontWeight:400,color:'#6B7280',fontSize:12}}>(* required)</span></div>
              ...
              {rawRows.length>0&&(
                <div style={{marginBottom:18}}>
                  <div style={{fontSize:13,fontWeight:700,color:'#111827',marginBottom:8}}>Step 3 ΓÇö Assign to Agent (optional)</div>
              ...
              {csvRows.length>0&&(
                <div style={{marginBottom:18}}>
                  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8,flexWrap:'wrap',gap:6}}>
                    <div style={{fontSize:13,fontWeight:700,color:'#111827'}}>Preview ΓÇö {csvRows.length} rows</div>
                    <span style={{fontSize:11,color:(missingCount>0||dupAgentCount>0)?'#DC2626':'#059669',fontWeight:600}}>
                      ...
                      {(missingCount>0||dupAgentCount>0)?' ΓÇö ':''}{trulyValidRows.length} valid
                    </span>
                  </div>
                      ...
                      <tbody>
                        {csvRows.slice(0,8).map(r=>(
                          <tr key={r._row} style={{borderTop:'1px solid #F3F4F6',opacity:(r._valid&&!r._dupAgent)?1:0.4}}>
                            ...
                            <td style={{padding:'7px 12px',fontWeight:500,color:'#111827'}}>{r.full_name||'ΓÇö'}</td>
                            <td style={{padding:'7px 12px',color:'#374151'}}>{r.mobile||'ΓÇö'}</td>
                            <td style={{padding:'7px 12px',color:'#374151'}}>{r.loan_amount?'Γé╣'+r.loan_amount:'ΓÇö'}</td>
                            <td style={{padding:'7px 12px',color:'#374151'}}>{r.application_id||'ΓÇö'}</td>
                            <td style={{padding:'7px 12px',color:'#374151',maxWidth:120,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{r.notes||'ΓÇö'}</td>
                            <td style={{padding:'7px 12px'}}>{
                              !r._valid
                                ...
                                :r._dupAgent
                                  ...
                                  :<span style={{background:'#D1FAE5',color:'#065F46',padding:'2px 7px',borderRadius:4,fontWeight:600}}>Γ£ô OK</span>
                            }</td>
                          </tr>
                        ))}
              ...
              {importResult&&(
                <div style={{background:importResult.fail===0?'#F0FFF4':'#FFFBEB',border:'1px solid '+(importResult.fail===0?'#86EFAC':'#FCD34D'),borderRadius:10,padding:14,marginBottom:16}}>
                  ...
                  <div style={{fontSize:13,color:importResult.fail===0?'#166534':'#78350F'}}>Γ£à {importResult.ok} leads imported successfully{importResult.fail>0?` ┬╖ ΓÜá∩╕Å ${importResult.fail} failed`:''}</div>
            ...
            <div style={{padding:'14px 20px',borderTop:'1px solid #E2E8F0',display:'flex',gap:10,justifyContent:'flex-end',flexShrink:0}}>
              ...
              {rawRows.length>0&&!importResult&&(
                <button onClick={handleImport} disabled={importing||!colMap.full_name||!colMap.mobile||trulyValidRows.length===0}
                  ...
                  {importing?'ImportingΓÇª':`Import ${trulyValidRows.length} Leads`}
                </button>
              )}
      ...
      <div onClick={()=>setViewLead(null)} style={{position:'fixed',inset:0,background:'rgba(15,23,42,0.55)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:9999,padding:16}}>
        <div onClick={e=>e.stopPropagation()} style={{background:'white',borderRadius:14,width:'100%',maxWidth:520,maxHeight:'85vh',overflowY:'auto',boxShadow:'0 20px 50px rgba(0,0,0,0.25)'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'18px 22px',borderBottom:'1px solid #EEF2F6'}}>
            <div>
              <div style={{fontSize:17,fontWeight:700,color:'##0F172A'}}>{viewLead.full_name||'ΓÇö'}</div>
              <div style={{fontSize:13,color:'#64748B'}}>{viewLead.mobile||'ΓÇö'}</div>
            </div>
            <button onClick={()=>setViewLead(null)} style={{border:'none',background:'#F1F5F9',width:32,height:32,borderRadius:8,cursor:'pointer',fontSize:18,color:'#475569'}}>├ù</button>
          </div>
          <div style={{padding:'18px 22px',display:'grid',gridTemplateColumns:'1fr 1fr',gap:'14px 18px'}}>
            {[
              ['Status',viewLead.status||'ΓÇö'],
              ['Previous Status',viewLead.previous_status||'ΓÇö'],
              ['Previous Agent',viewLead.previous_agent_name||'ΓÇö'],
              ...
              ['Sheet No.',viewLead.sheet_number||'ΓÇö'],
              ['City',viewLead.city||'ΓÇö'],
              ['Loan Amount',viewLead.loan_amount?'Γé╣'+Number(viewLead.loan_amount).toLocaleString('en-IN'):'ΓÇö'],
              ['Monthly Salary',viewLead.monthly_salary?'Γé╣'+Number(viewLead.monthly_salary).toLocaleString('en-IN'):'ΓÇö'],
              ['Company',viewLead.company_name||'ΓÇö'],
              ['Application ID',viewLead.application_id||'ΓÇö'],
              ['Date',viewLead.created_at?new Date(viewLead.created_at).toLocaleDateString('en-IN',{timeZone:IST_TZ}):'ΓÇö'],
            ].map(([k,v])=>(
              <div key={k}>
            ...
            <div style={{gridColumn:'1 / -1'}}>
              ...
              <div style={{fontSize:13.5,color:'#334155',whiteSpace:'pre-wrap',lineHeight:1.5}}>{viewLead.notes||'ΓÇö'}</div>
            </div>
              ...
              <div style={{gridColumn:'1 / -1'}}>
                ...
                {(adminObligations[viewLead.id]||[]).map((o,i)=>(
                  <div key={i} style={{display:'flex',justifyContent:'space-between',padding:'6px 0',borderBottom:'1px solid #F1F5F9',fontSize:13,color:'#334155'}}>
                    <span>{o.bank_name||'ΓÇö'} {o.loan_type?'('+o.loan_type+')':''}</span>
                ...
                <div style={{gridColumn:'1 / -1'}}>
                  ...
                  {reassignments.map((a,i)=>(
                    <div key={a.id||i} style={{display:'flex',justifyContent:'space-between',padding:'6px 0',borderBottom:'1px solid #F1F5F9',fontSize:13,color:'#334155'}}>
                      <span>{a.previous_agent_name||'ΓÇö'} ΓåÆ {a.assigned_to_name||'ΓÇö'}</span>
                      <span style={{fontWeight:600}}>
                        {a.created_at?new Date(a.created_at).toLocaleString('en-IN',{dateStyle:'medium',timeStyle:'short',timeZone:IST_TZ}):'ΓÇö'}
  ...
  // admin uses main layout ΓÇö no early return

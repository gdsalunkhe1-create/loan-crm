            ...
            <div>
              ...
              {selected.size>0&&(
                <div style={{background:'#185FA5',color:'white',padding:'10px 14px',borderRadius:8,marginBottom:10,display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}}>
                  ...
                  <button className='btn' style={{background:'white',color:'#185FA5',fontSize:13,padding:'5px 14px'}} onClick={async()=>{ if(!assignTo||selected.size===0)return; setAssigning(true); try{ const selLeads=adminLeads.filter(l=>selected.has(l.id)); const agent=users.find(u=>u.id===assignTo); const ids=[...selected]; const now=new Date().toISOString(); const byStatus={}; selLeads.forEach(l=>{const s=l.status||'New';if(!byStatus[s])byStatus[s]=[];byStatus[s].push(l.id)}); for(const[oldStatus,sIds] of Object.entries(byStatus)){const upd={assigned_to:assignTo,status:'New',assigned_at:now};if(oldStatus!=='New')upd.previous_status=oldStatus;for(let i=0;i<sIds.length;i+=100)await supabase.from('leads').update(upd).in('id',sIds.slice(i,i+100));} const logEntries=selLeads.map(l=>({lead_id:l.id,lead_name:l.full_name||'',action:l.assigned_to?'Reassigned':'Assigned',assigned_to:assignTo,assigned_to_name:agent?.full_name||'',assigned_by:profile?.id||null,assigned_by_name:profile?.full_name||'Admin',previous_agent_id:l.assigned_to||null,previous_agent_name:users.find(u=>u.id===l.assigned_to)?.full_name||null})); await supabase.from('activity_log').insert(logEntries); showApToast(ids.length+' lead'+(ids.length>1?'s':'')+' assigned to '+agent?.full_name); try{await supabase.from('notifications').insert([{type:'leads_assigned',agent_id:assignTo,agent_name:'Admin',message:`📥 ${ids.length} new lead${ids.length>1?'s':''} assigned to you`}])}catch(e){console.error('[assign notify]',e)} setSelected(new Set()); setAssignTo(''); fetchLeads(); fetchActivityFull() }catch(err){showApToast('Error: '+err.message,'error')} setAssigning(false)}} disabled={assigning||!assignTo}>{assigning?'Assigning…':'Assign'}</button>

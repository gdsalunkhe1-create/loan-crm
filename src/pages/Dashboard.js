...
<td style={{padding:'8px 6px',verticalAlign:'middle',fontSize:11.5,color:'#475569',overflow:'hidden',whiteSpace:'nowrap'}} title={agent?.full_name||''}><div style={{overflow:'hidden',textOverflow:'ellipsis'}}>{isMirrorRow?(users.find(u=>u.id===leadAgentF)?.full_name||agent?.full_name):(agent?.full_name||<span style={{color:'#CBD5E1'}}>Unassigned</span>)}</div>{isMirrorRow&&<span style={{display:'inline-block',fontSize:9,background:'#EFF6FF',color:'#1D4ED8',padding:'1px 5px',borderRadius:4,fontWeight:700,marginTop:2}}>MIRROR</span>}</td>

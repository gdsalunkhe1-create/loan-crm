...
<td style={{padding:'11px 12px',verticalAlign:'middle',fontSize:12.5,color:'#475569'}}>{agent?.full_name||<span style={{color:'#CBD5E1'}}>Unassigned</span>}</td>
<td style={{padding:'11px 12px',verticalAlign:'middle',fontSize:12.5,color:'#1E293B',fontWeight:500,whiteSpace:'nowrap'}}>{l.loan_amount?'₹'+Number(l.loan_amount).toLocaleString('en-IN'):<span style={{color:'#CBD5E1'}}>—</span>}</td>
<td style={{padding:'11px 12px',verticalAlign:'middle',fontSize:11.5,color:'#6366F1'}}>{obs.length>0?obs.map(o=>o.obligation_type).join(', '):<span style={{color:'#CBD5E1'}}>—</span>}</td>
<td style={{padding:'11px 12px',verticalAlign:'middle'}}>{foir!==null?<span style={{display:'inline-block',padding:'3px 9px',borderRadius:6,fontSize:11.5,fontWeight:600,background:foir>50?'#FEF2F2':'#F0FDF4',color:foir>50?'#DC2626':'#16A34A'}}>{foir}%</span>:<span style={{color:'#CBD5E1'}}>—</span>}</td>
<td style={{padding:'11px 12px',verticalAlign:'middle',fontSize:12.5,color:'#185FA5',fontWeight:500,whiteSpace:'nowrap'}}>{totalEMI>0?'₹'+totalEMI.toLocaleString('en-IN'):<span style={{color:'#CBD5E1'}}>—</span>}</td>
<td style={{padding:'11px 12px',verticalAlign:'middle',fontSize:11.5,color:'#64748B',maxWidth:200,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}} title={lastNote}>{lastNote||<span style={{color:'#CBD5E1'}}>—</span>}</td>
<td style={{padding:'11px 12px',verticalAlign:'middle',whiteSpace:'nowrap'}}>
  <div style={{display:'flex',alignItems:'center',gap:8}}>
    ...
    <button onClick={()=>{setReassignTarget(l);setReassignTo('')}} title='Reassign (original agent keeps access)' style={{display:'inline-flex',alignItems:'center',justifyContent:'center',width:30,height:30,borderRadius:8,background:'#EFF6FF',border:'1px solid #BFDBFE',color:'#1D4ED8',fontSize:15,cursor:'pointer'}}>↩</button>
    <button onClick={()=>handleUnassignLead(l)} title='Unassign agent' style={{display:'inline-flex',alignItems:'center',justifyContent:'center',width:30,height:30,borderRadius:8,background:'#FFF7ED',border:'1px solid #FED7AA',color:'#C2410C',fontSize:15,cursor:'pointer'}}>⊘</button>
    <button onClick={()=>handleDeleteLead(l)} title='Delete lead permanently' style={{display:'inline-flex',alignItems:'center',justifyContent:'center',width:30,height:30,borderRadius:8,background:'#FEF2F2',border:'1px solid #FECACA',color:'#DC2626',fontSize:14,cursor:'pointer'}}>🗑</button>
  </div>
</td>

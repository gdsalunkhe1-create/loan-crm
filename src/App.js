    ...
    <>
      {showInstall && (
        <div style={{position:'fixed',top:0,left:0,right:0,zIndex:9999,display:'flex',justifyContent:'space-between',alignItems:'center',padding:'12px 14px',background:'#185FA5',color:'white',boxShadow:'0 4px 16px rgba(0,0,0,0.16)'}}>
          <div style={{fontSize:13,fontWeight:600}}>Install CALL-Q PRO CRM for a full-screen experience.</div>
          <button onClick={handleInstallClick} style={{border:'none',background:'white',color:'#185FA5',fontWeight:700,padding:'10px 14px',borderRadius:'8px',cursor:'pointer'}}>Install</button>
        </div>
      )}
      {showIOSInstall && (
        <div style={{position:'fixed',top:0,left:0,right:0,zIndex:9999,display:'flex',justifyContent:'space-between',alignItems:'center',padding:'12px 14px',background:'#111827',color:'white',boxShadow:'0 4px 16px rgba(0,0,0,0.16)'}}>
          <div style={{fontSize:13,fontWeight:600}}>Add CALL-Q PRO CRM to your Home Screen: tap Share then Add to Home Screen.</div>
          <button onClick={() => setShowIOSInstall(false)} style={{border:'none',background:'white',color:'#111827',fontWeight:700,padding:'10px 14px',borderRadius:'8px',cursor:'pointer'}}>Dismiss</button>
        </div>
      )}
      <BrowserRouter>
        <Routes>
